...
export default function NewsFeed() {
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: "11px", lineHeight: 1.5, opacity: 0.9 }}>
          ...
          client tries GDELT again. Reload the app or wait for the next refresh
          (every 5 min).
        </div>
      </div>
    );
  ...
  const cats = [
    "all",
    ...Array.from(
      new Set(articles.map((a) => a.cat ?? "crypto").filter(Boolean)),
    ),
  ];
  const visible =
  ...
  return (
    ...
    >
      ...
      >
        {a.src && (
          <span
            style={{ fontSize: "10px", color: "var(--text2)", fontWeight: 700 }}
          >
            {a.src}
          </span>
        )}
