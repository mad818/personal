...
export async function getCheckpoint(
  label: string,
): Promise<SystemSnapshot | undefined> {
...
export async function getContradictions(content: string): Promise<Memory[]> {
  ...
  const candidates = all.filter((mem) => {
    ...
    const newHasNegation =
      /\b(not|no longer|deprecated|removed|disabled|false|never)\b/.test(
        content.toLowerCase(),
      );
    const memHasNegation =
      /\b(not|no longer|deprecated|removed|disabled|false|never)\b/.test(
        memLower,
      );
    ...
    const hasConflictingNums =
      newNums.length > 0 &&
      memNums.length > 0 &&
      newNums.some((n) =>
        memNums.some((m) => m !== n && parseFloat(m) !== parseFloat(n)),
      );
...
export async function importMemories(
  ...
  for (const mem of parsed.memories) {
    if (!mem.id || !mem.content || !mem.type) {
      skipped++;
      continue;
    }
    ...
    const safe: Memory = {
      ...
      relevanceScore:
        typeof mem.relevanceScore === "number"
          ? Math.min(1, Math.max(0, mem.relevanceScore))
          : 1.0,
      source: mem.source,
...
export async function getMemoryStats(): Promise<
  Record<MemoryType | "total", number>
> {
  const all = await idbGetAll<Memory>(STORE_NAME);
