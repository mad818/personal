...
function chipStyle(color: ChipColor): React.CSSProperties {
  const map: Record<ChipColor, { bg: string; border: string; text: string }> = {
    green:  { bg: 'rgba(16,185,129,.12)',  border: 'rgba(16,185,129,.35)',  text: 'var(--fhi)' },
    yellow: { bg: 'rgba(245,158,11,.12)',  border: 'rgba(245,158,11,.35)',  text: 'var(--fmd)' },
    red:    { bg: 'rgba(239,68,68,.12)',   border: 'rgba(239,68,68,.35)',   text: 'var(--flo)' },
    muted:  { bg: 'var(--surf3)',          border: 'var(--border)',         text: 'var(--text3)' },
  }
  const c = map[color]
  return {
    display: 'inline-flex',
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, alignItems: 'center' }}>
        ...
        <span style={chipStyle(evalColor)}>
          ...
          {ageLabel && !loading && (
            <span style={{ fontWeight: 400, opacity: .7, fontSize: 10 }}>({ageLabel})</span>
          )}
        </span>
