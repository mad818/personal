...
const CRAB: Record<string, string[]> = {
  ...
  thinking: [
    'c  cccccc   ','cc cccccccc ',' cccyeyccc  ','cccccccccccc',
    'ccCccccccCcc',' cccccccccc ','  cc  cc    ','   c  cc    ',
  ],
  happy: [
  ...
  working: [
    '   cccccc   ','  cccccccc  ','cccyeyccccc ','cccccccccccc',
    'cCccccccccCc',' cccccccccc ','cc  cc  cc  ','cc  cc  cc  ',
  ],
  excited: [
    'c  cccccc  c',' c cccccc c ','  ccyRyccc  ',' cccccccccc ',
    'ccCccgcccCcc',' gccccccccg ','  cc  cc    ',' c c  cc c  ',
  ],
  error: [
    '   xxxxxx   ','  cccccccc  ',' cccxexxcc  ','xxccccccccxx',
    'ccCccccccCcc',' cccccccccc ','  cc  cc  c ','  c   cc   c',
  ],
  success: [
    'g  cccccc  g',' g cccccc g ','  ccgegccc  ','ccccggggcccc',
    'ccCcggggcCcc',' gccccccccg ','  cc  cc  c ',' g c  cc g  ',
  ],
}
...
function buildComment(
  ...
  const snippets: Record<string, string[]> = {
    ...
    '/ops': [
      ...
      worldRisk > 5 ? 'Elevated geopolitical tension detected.' : 'Threat posture: stable.',
      `GDELT feed active.`,
    ],
    '/intel': [
...
export default function AgentStatusBar() {
  ...
  const cves        = useStore(s => (s as unknown as { cves?: unknown[] }).cves ?? [])
  ...
  useEffect(() => {
    const criticalCves = Array.isArray(cves) ? cves.filter((c: unknown) => {
  ...
  useEffect(() => {
    const refresh = () => setComment(buildComment(
      pathname ?? '/home',
      articles.length,
    ...
    >
      ...
      {expanded && (
        <div style={{
          ...
          <span style={{ color: agentCfg.color, fontWeight: 800, fontSize: '10px' }}>
            {agentCfg.name}
          </span>
          <br />
          {comment}
      ...
      <div
        ...
        onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 4px 28px rgba(0,0,0,.55), 0 0 18px ${agentCfg.color}44` }}
        onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 4px 24px rgba(0,0,0,.45), 0 0 12px ${agentCfg.color}22` }}
      >
        {/* Crab */}
        <div style={{
          animation: emotion === 'working' || emotion === 'excited' ? 'miniCrabBob .6s ease-in-out infinite alternate' : 'none',
        }}>
          <Sprite rows={CRAB[emotion]} scale={0.75} />
        </div>

        {/* Agent sprite */}
        <div style={{ animation: 'miniAgentBob 1.8s ease-in-out infinite alternate' }}>
