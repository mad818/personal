...
export class WebSocketManager {
  ...
  private listeners: Set<(status: WSStatus, msg?: WSMessage) => void> =
    new Set();
    ...
    try {
      switch (type) {
        case "sensor:data":
          eventBus.emit(
            "sensor:data",
            payload as Parameters<typeof eventBus.emit<"sensor:data">>[1],
          );
          break;
        case "camera:alert":
          eventBus.emit(
            "camera:alert",
            payload as Parameters<typeof eventBus.emit<"camera:alert">>[1],
          );
          break;
        case "drone:telemetry":
          eventBus.emit(
            "drone:telemetry",
            payload as Parameters<typeof eventBus.emit<"drone:telemetry">>[1],
          );
          break;
        case "vehicle:telemetry":
          eventBus.emit(
            "vehicle:telemetry",
            payload as Parameters<typeof eventBus.emit<"vehicle:telemetry">>[1],
          );
          break;
        case "mqtt:message":
          eventBus.emit(
            "mqtt:message",
            payload as Parameters<typeof eventBus.emit<"mqtt:message">>[1],
          );
          break;
        case "system:health":
          eventBus.emit(
            "system:health",
            payload as Parameters<typeof eventBus.emit<"system:health">>[1],
          );
          break;
      }
    } catch {
