      ...
      priority:
        i === 0
          ...
          : i <= 2
            ? ("Medium" as const)
            : ("Low" as const),
      improvement: `+${Math.round((0.9 - s.successRate) * 100)}%`,
      category: s.category,
    }));
}
...
function MetricTile({
  ...
  return (
    ...
    >
      ...
      {sub && (
        <div
          style={{ fontSize: "9px", color: "var(--text3)", marginTop: "3px" }}
        >
          {sub}
        </div>
      )}
...
export default function SystemBrain() {
  ...
  const hubSkills = useMemo(
    () => getHubSkills(graph, skills, 5, 2),
    [graph, skills],
  );
  const clusters = useMemo(
    () => getSkillClusters(graph, skills, 5),
    [graph, skills],
  );

  // Build improvement queue from real skill data
  const improvementQueue = useMemo(
    () => buildImprovementQueue(skills),
    [skills],
  );
  const pendingImprovements = improvementQueue.filter(
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          >
            ...
            <MetricTile
              ...
              color="var(--gold)"
            />
            <MetricTile label="Uptime" value={uptimeStr} color="var(--text)" />
            <MetricTile
              label="Memory Used"
              value={`${memUsed} MB`}
