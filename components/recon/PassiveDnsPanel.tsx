...
async function fetchCirclPdns(domain: string): Promise<string> {
  try {
    ...
    const records: PdnsRecord[] = text.trim().split('\n')
      .filter(Boolean)
      .map(line => { try { return JSON.parse(line) as PdnsRecord } catch { return null } })
      .filter((x): x is PdnsRecord => x !== null)

    if (!records.length) return '<span style="color:var(--text3)">No passive DNS records found</span>'
    ...
    byType.forEach((recs, type) => {
      ...
      recs.slice(0, 10).forEach(rec => {
        const first = rec.time_first ? new Date(rec.time_first * 1000).toISOString().slice(0, 10) : ''
        const last  = rec.time_last  ? new Date(rec.time_last  * 1000).toISOString().slice(0, 10) : ''
...
async function fetchReverseIp(ip: string): Promise<string> {
  try {
    ...
    if (!hosts.length) return '<span style="color:var(--text3)">No co-hosted domains found</span>'
...
export default function PassiveDnsPanel() {
  ...
  return (
    <div>
      <p style={{ fontSize: '12px', color: 'var(--text2)', marginBottom: '14px', marginTop: 0 }}>
        Look up historical DNS resolutions and co-hosted domains. Free — no API key required.
      </p>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '10px', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => { setTarget(''); setPdns(''); setRevIp('') }}
      ...
      {detectedType && (
        <div style={{ fontSize: '10px', color: 'var(--text3)', marginBottom: '14px' }}>
          Detected: <span style={{ color: 'var(--accent)', fontWeight: 700 }}>{detectedType.toUpperCase()}</span>
          {detectedType === 'domain'
      ...
      {(pdns || revIp) && (
        <div style={{
          display: 'grid',
          gridTemplateColumns: pdns && revIp ? 'repeat(auto-fill, minmax(300px, 1fr))' : '1fr',
          gap: '16px',
          marginTop: '8px',
        }}>
          {pdns && (
      ...
      {!pdns && !revIp && !loading && (
        <p style={{ fontSize: '10px', color: 'var(--text3)', marginTop: '12px' }}>
          Sources: <strong>CIRCL Passive DNS</strong> (circl.lu) — historical A, MX, NS, TXT records with
          first/last seen dates · <strong>HackerTarget</strong> — reverse IP lookup showing co-hosted
