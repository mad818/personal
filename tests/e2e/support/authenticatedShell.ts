...
export async function seedAuthenticatedShell(page: Page) {
  ...
  await page.context().addCookies([
    {
      name: "nexus_session_token",
      value: validToken,
      url: getCookieUrl(),
      sameSite: "Lax",
      secure: getCookieUrl().startsWith("https://"),
    },
  ]);

  await page.addInitScript((token: string) => {
    try {
      window.sessionStorage.setItem("nexus_session_token", token);
    } catch {
      // sessionStorage can be unavailable in constrained browser modes.
    }
  }, validToken);
}
