...
const SWE_SCORES: Record<string, string> = {
  ...
  scaleway: "S+ 72.2%", qwen: "S+ 70.6%", huggingface: "S 62.0%",
  codestral: "B+ 34.0%", googleai: "B 22.0%", cloudflare: "S+ 76.8%",
  perplexity: "A 45.0%", openrouter: "S+ 70.6%", google: "—",
  minimax: "—", anthropic: "S+ 75.0%", openai: "—",
};
...
function hasKey(provider: string): boolean {
  switch (provider) {
    ...
    case "nvidia":      return isConfiguredSecretValue(process.env.NVIDIA_API_KEY);
    case "hyperbolic":  return isConfiguredSecretValue(process.env.HYPERBOLIC_API_KEY);
    case "together":    return isConfiguredSecretValue(process.env.TOGETHER_API_KEY);
    case "siliconflow": return isConfiguredSecretValue(process.env.SILICONFLOW_API_KEY);
    case "zai":         return isConfiguredSecretValue(process.env.ZAI_API_KEY);
    case "iflow":       return isConfiguredSecretValue(process.env.IFLOW_API_KEY);
    case "deepinfra":   return isConfiguredSecretValue(process.env.DEEPINFRA_API_KEY);
    case "fireworks":   return isConfiguredSecretValue(process.env.FIREWORKS_API_KEY);
    case "scaleway":    return isConfiguredSecretValue(process.env.SCALEWAY_API_KEY);
    case "qwen":        return isConfiguredSecretValue(process.env.DASHSCOPE_API_KEY);
    case "huggingface": return isConfiguredSecretValue(process.env.HUGGINGFACE_API_KEY);
    case "codestral":   return isConfiguredSecretValue(process.env.CODESTRAL_API_KEY);
    case "googleai":    return isConfiguredSecretValue(process.env.GOOGLE_AI_KEY);
    case "cloudflare":  return isConfiguredSecretValue(process.env.CLOUDFLARE_API_TOKEN) && isConfiguredSecretValue(process.env.CLOUDFLARE_ACCOUNT_ID);
    case "perplexity":  return isConfiguredSecretValue(process.env.PERPLEXITY_API_KEY);
    case "openrouter":  return isConfiguredSecretValue(process.env.OPENROUTER_API_KEY);
    case "google":      return isConfiguredSecretValue(process.env.GOOGLE_AI_KEY);
    case "minimax":     return isConfiguredSecretValue(process.env.MINIMAX_API_KEY);
    case "anthropic":   return isConfiguredSecretValue(process.env.ANTHROPIC_API_KEY);
    case "openai":      return isConfiguredSecretValue(process.env.OPENAI_API_KEY);
...
function computeCooldownRemaining(circuitOpenAt: number, consecutiveFails: number): number {
  if (circuitOpenAt === 0) return 0;
  const STEPS = [2, 5, 15, 30].map((m) => m * 60_000);
  const cd  = STEPS[Math.min(consecutiveFails - 1, STEPS.length - 1)] ?? STEPS[0];
  ...
  const providers: ProviderHealth[] = ALL_PROVIDERS_ORDER.map((name) => {
    ...
    return {
      ...
      cooldownRemainingMs: circuitInfo
        ? computeCooldownRemaining(circuitInfo.circuitOpenAt, circuitInfo.consecutiveFails)
        : 0,
