...
export async function GET() {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      if (r.status === 401) {
        return NextResponse.json(
          {
            error:
              "Invalid FIRMS_MAP_KEY. Verify your key at https://firms.modaps.eosdis.nasa.gov/api/",
            fires: [],
          },
          { status: 401 },
        );
      ...
      if (r.status === 429) {
        return NextResponse.json(
          {
            error:
              "NASA FIRMS rate limit exceeded. Try again in a few minutes.",
            fires: [],
          },
          { status: 429 },
        );
    ...
    if (
      csvText.includes("Transaction Rejected") ||
      csvText.includes("Invalid MAP_KEY")
    ) {
      return NextResponse.json(
        {
