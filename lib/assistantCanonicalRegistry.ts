...
export const CANONICAL_SIMPLE_FOCUS_ROUTES: Record<string, readonly string[]> = {
  ...
  "/command": ["runtime-efficiency", "agent-health", "memory-spine"],
  "/vehicle": [
...
> = {
  ...
  "/cyber": {
    defaultView: "triage",
    allowedViews: ["triage", "matrix", "cves", "otx", "cisa", "drone"],
    focusToView: {
      "cyber-triage": "triage",
      "cyber-matrix": "matrix",
      "cyber-cves": "cves",
...
export function normalizeCanonicalVaultParams(params: URLSearchParams) {
  ...
  if (graphAudit) focus = "vault-graph-focus";

  if (focus && !CANONICAL_VAULT_FOCUSES.has(focus as never)) {
    focus = compiledFilter ? "vault-compiled-pages" : graphAudit ? "vault-graph-focus" : null;
  }
