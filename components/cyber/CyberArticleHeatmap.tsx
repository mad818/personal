      ...
      {open && (
        <div
          ...
          style={{
            ...
            animation: "cyberFadeIn .15s ease",
          }}
        />
      )}
      <div
      ...
      >
        ...
        >
          {articles.map((a) => {
            ...
            const tColor =
              score > 0.1 ? "#ef4444" : score < -0.1 ? "#10b981" : "#6875a0";
            const tLabel =
              score > 0.1
                ? "High Threat"
                : score < -0.1
...
function HeatCell({
  ...
  const avgScore =
    count === 0
      ? 0
      : articles.reduce(
          (sum, a) => sum + threatScore(a.title + " " + (a.desc ?? "")),
          0,
        ) / count;
  const tColor =
    avgScore > 0.1 ? "#ef4444" : avgScore < -0.1 ? "#10b981" : "#6875a0";
  const tLabel =
    avgScore > 0.1 ? "High Threat" : avgScore < -0.1 ? "Patched" : "Monitoring";
  const freshest = articles.length
    ? articles.reduce((a, b) => (new Date(a.date) > new Date(b.date) ? a : b))
    : null;
  const glowPct = Math.round((0.08 + intensity * 0.3) * 100);
...
export default function CyberArticleHeatmap() {
  ...
  const maxCount = Math.max(
    1,
    ...CYBER_TOPICS.map((t) => byTopic[t.key].length),
  );
  const panelArticles = activeTopic ? (byTopic[activeTopic.key] ?? []) : [];
  const activeTopics = CYBER_TOPICS.filter(
    (t) => byTopic[t.key].length > 0,
  ).length;

  if (!cyberArticles.length)
    return (
      <div
        style={{
          padding: "40px",
      ...
      >
        ...
        {articleStatus.lastError && articles.length === 0
          ...
          : articlesLoaded
            ? "No cyber stories matched the current verified news mix."
    ...
    <>
      ...
      >
        <span style={{ fontSize: "11px", color: "var(--text3)" }}>
          {cyberArticles.length} threat signals · {activeTopics} active
          categories
        </span>
        <span
          style={{
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
          <div
            style={{
              ...
              height: "4px",
              borderRadius: "2px",
              background: "linear-gradient(to right, var(--surf3), #ef4444)",
            }}
          />
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: "12px" }}>
          ...
          <span style={{ fontSize: "9px", color: "#6875a0", fontWeight: 700 }}>
            ● Monitoring
          </span>
