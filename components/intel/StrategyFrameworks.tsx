...
const VRIO_DIMS = [
  {
    key: "valuable",
    label: "Valuable",
    desc: "Does it exploit an opportunity or neutralise a threat?",
  },
  { key: "rare", label: "Rare", desc: "Is it controlled by few firms?" },
  {
    key: "imitable",
    label: "Hard to Imitate",
    desc: "Is it costly or difficult to copy?",
  },
  {
    key: "organized",
    label: "Organized",
    desc: "Is the firm set up to capture the value?",
  },
] as const;
...
export default function StrategyFrameworks() {
  ...
  const analyse = useCallback(async () => {
    ...
    if (active === "porter") {
      const scores = PORTER_FORCES.map(
        (f) => `${f.label}: ${porterScores[f.key]}/5`,
      ).join(", ");
      ...
      const dims = VRIO_DIMS.map(
        (d) => `${d.label}: ${vrioChecks[d.key] ? "Yes" : "No"}`,
      ).join(", ");
      ...
      const matrix = BCG_QUADRANTS.map(
        (q) => `${q.label}: ${bcgItems[q.key] || "empty"}`,
      ).join(" | ");
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {PORTER_FORCES.map((f) => (
            ...
            >
              ...
              >
                {porterScores[f.key] >= 4
                  ? "High"
                  : porterScores[f.key] <= 2
                    ? "Low"
                    : "Med"}
              </span>
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          >
            ...
            <input
              type="text"
              value={vrioResource}
              onChange={(e) => setVrioResource(e.target.value)}
              placeholder="e.g. proprietary data, brand, tech stack…"
              style={inputStyle}
            />
          </label>
          {VRIO_DIMS.map((d) => (
            ...
            >
              <input
                type="checkbox"
                checked={!!vrioChecks[d.key]}
                onChange={() =>
                  setVrioChecks((p) => ({ ...p, [d.key]: !p[d.key] }))
                }
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          ...
          >
            ...
            <input
              type="text"
              value={jtbdJob}
              onChange={(e) => setJtbdJob(e.target.value)}
          ...
          >
            ...
            <input
              type="text"
              value={jtbdPain}
              onChange={(e) => setJtbdPain(e.target.value)}
              placeholder="What frustration or obstacle exists today?"
              style={inputStyle}
            />
          </label>
          <label
          ...
          >
            ...
            <input
              type="text"
              value={jtbdGain}
              onChange={(e) => setJtbdGain(e.target.value)}
