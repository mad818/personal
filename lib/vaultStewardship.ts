...
export interface VaultStewardshipSnapshot {
  ...
  topUnderlinkedTitles: string[];
  topNoBacklinkTitles: string[];
...
export function buildVaultStewardshipSnapshot(input: {
  ...
  const underlinkedCount = lint?.underlinkedPages.length ?? 0;
  ...
  const topUnderlinkedTitles = (lint?.underlinkedPages ?? [])
    .slice(0, 3)
    .map((id) => nodeTitleMap.get(id) ?? id);
  const topNoBacklinkTitles = (lint?.noBacklinkPages ?? [])
    .slice(0, 3)
    .map((id) => nodeTitleMap.get(id) ?? id);
  ...
  if (underlinkedCount > 0) {
    priorities.push(
      `Strengthen ${underlinkedCount} underlinked archive node${underlinkedCount === 1 ? "" : "s"} before they drift into isolated recall paths.`,
    );
  }
  if (noBacklinkCount > 0) {
    priorities.push(
      `Add durable backlinks to ${noBacklinkCount} archive artifact${noBacklinkCount === 1 ? "" : "s"} so reuse compounds instead of stalling at one-way capture.`,
    );
  }
  ...
  const detail =
    totalArtifacts === 0
      ...
      : `Orphans, stale items, underlinked nodes, and thin topics are archive-health issues, not cosmetic ones. Reverse-engineering prep notes and promoted analyst briefs deserve the same upkeep as any other durable artifact, while restricted compiled pages still count for posture even when their bodies stay protected.`;
