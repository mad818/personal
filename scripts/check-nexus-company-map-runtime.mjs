...
assert.match(COMPANY_SKILL_SOURCES.find((source) => source.id === "pliny-st3gg")?.codexPath ?? "", /browser-local PNG\/JPEG\/PDF tail detector/i);
...
assert.ok(NEXUS_COMPANY_DEPARTMENTS.find((department) => department.id === "legal-trust")?.sourceIds.includes("pliny-st3gg"));
