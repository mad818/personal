...
export function observeSkill(input: ObserveInput): LearningEvent {
  const { skillId, taskInput, outcome, errorMsg, feedback = 0 } = input;
  ...
  if (skill) {
    ...
    const newFailureCount =
      (skill.failureCount ?? 0) + (outcome === "failure" ? 1 : 0);
    ...
    const log = getLearningLog();
    const recent20 = log.filter((e) => e.skillId === skillId).slice(-20);
...
export async function inspectSkill(skillId: string): Promise<InspectionResult> {
  ...
  function heuristicAnalysis(): InspectionResult {
    ...
    const pattern = topWord
      ? `Recurring token: "${topWord[0]}" (${topWord[1]} occurrences)`
      : "Diverse failure types";
    const urgency =
      failures.length >= 10 ? "high" : failures.length >= 5 ? "medium" : "low";
    return {
...
export function evaluateAmendment(amendmentId: string): EvaluationResult {
  ...
  const skill = (() => {
    ...
    return all.find((s) =>
      (s.amendments ?? []).some((a) => a.id === amendmentId),
    );
  })();
  ...
  const log = getLearningLog();
  const preEvents = log
    .filter((e) => e.skillId === skill.id && e.timestamp < amendment.proposedAt)
    .slice(-50);
  const preSuccess = preEvents.filter(
    (e) => e.type === "improved" || e.type === "pattern_detected",
  ).length;
  const preRate =
    preEvents.length > 0
      ? preSuccess / preEvents.length
  ...
  return {
    improved,
    metricDelta: delta,
    action: improved ? "commit" : "rollback",
  };
}
  ...
  const scored: RouteResult[] = skills.map((skill) => {
    ...
    const triggerMatches = (skill.triggerPhrases ?? []).filter((tp) => {
      ...
      return (
        query.includes(tp2) ||
        tp2.includes(query) ||
        tokens.some((t) => tp2.includes(t))
      );
    }).length;
    if (triggerMatches > 0) {
      score += triggerMatches * 20;
      matchedBy = "triggerPhrases";
    }
    ...
    if (descMatches > 0) {
      score += descMatches * 3;
      if (score < 30) matchedBy = "description";
    }
    ...
    const qualityMultiplier =
      1 + skill.level / 200 + (skill.successRate ?? 0.5) / 10;
