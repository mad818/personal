...
function listTree(
  ...
  const SKIP = new Set(['node_modules', '.next', '.git', 'dist', '.turbo', 'coverage'])
...
function countFiles(rel: string, ext = '.tsx'): number {
  ...
  try {
    return readdirSync(abs).filter(f => f.endsWith(ext) || f.endsWith('.ts')).length
  } catch { return 0 }
}
...
export async function GET(req: NextRequest) {
  ...
  if (section === 'tree') {
    ...
    const sourceDirs = ['app', 'components', 'lib', 'store', 'hooks', '.claude/skills']
    const tree: string[] = []
