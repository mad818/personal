...
const PHASE_ORDER: OperationalPhase[] = [
  'idle', 'interpreting', 'planning', 'executing', 'validating', 'responding', 'done',
]
  ...
  return (
    <div style={{
      ...
      <span style={{
        ...
        color: '#304060', letterSpacing: '2px', marginRight: '12px',
        whiteSpace: 'nowrap',
      }}>
        OP:
      </span>
          ...
          <div key={ph.id} style={{ display: 'flex', alignItems: 'center' }}>
            ...
            {i < PHASES.length - 1 && (
              <div style={{
                ...
                background: isDone && phaseIndex(PHASES[i + 1].id) <= curIdx
                  ? '#00FF6644'
                  : '#1A2040',
      ...
      {currentPhase !== 'done' && (
        <ElapsedTimer startedAt={phaseStartedAt} />
      )}
    </div>
  )
}
