...
const sourceIntelligence = readRequired(
  "lib",
  "homefrontSourceIntelligence.ts",
);
...
const aiOsint = JSON.parse(
  readRequired("docs", "ideas", "source-parity", "ai-osint.json"),
);
const agentShield = JSON.parse(
  readRequired("docs", "ideas", "source-parity", "agentshield.json"),
);
const owaspApts = JSON.parse(
  readRequired("docs", "ideas", "source-parity", "owasp-apts.json"),
);
...
console.log(
  `ok ai-exposure-assimilation (${AI_EXPOSURE_PACK_COUNT(exposureLib)} packs, AgentShield + APTS governance contracts retained)`,
);

function AI_EXPOSURE_PACK_COUNT(source) {
  const matches = source.match(
    /id: "(llm-endpoint|leaked-key|vector-store|mcp-exposure|unsafe-agent)"/g,
  );
