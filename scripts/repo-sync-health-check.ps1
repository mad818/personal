...
if ($blocked) {
  ...
  Write-Host "  4. Rerun npm run repo:sync:health."
  Write-Host "  5. Then run npm run handoff:pull and git fetch --all --prune."
} else {
  Write-Host "Next safe steps:"
  Write-Host "  1. Run npm run handoff:pull."
  Write-Host "  2. Run git fetch --all --prune."
  Write-Host "  3. Run git status --short --branch."
