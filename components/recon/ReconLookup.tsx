...
// ── types ─────────────────────────────────────────────────────────────────────
type TargetType = 'auto' | 'domain' | 'ip' | 'email' | 'hash' | 'url' | 'username'

type PanelKey = 'rdap' | 'dns' | 'certs' | 'geo' | 'subdomains' | 'dnssec' |
                'emailrep' | 'username' | 'hibp' | 'vt' | 'shodan'

type PanelState = Record<PanelKey, string>
type LoadingMap  = Record<PanelKey, boolean>

const EMPTY_PANELS: PanelState = {
  rdap: '', dns: '', certs: '', geo: '', subdomains: '', dnssec: '',
  emailrep: '', username: '', hibp: '', vt: '', shodan: '',
}
const EMPTY_LOADING: LoadingMap = {
  rdap: false, dns: false, certs: false, geo: false, subdomains: false, dnssec: false,
  emailrep: false, username: false, hibp: false, vt: false, shodan: false,
}
...
async function fetchCerts(domain: string): Promise<string> {
  try {
    ...
    if (!r.ok) throw new Error(`HTTP ${r.status}`)
    const data: {common_name?: string; not_before?: string; not_after?: string; issuer_name?: string}[] = await r.json()
...
async function fetchSubdomains(domain: string): Promise<string> {
  try {
    ...
    if (!lines.length) return '<span style="color:var(--text3)">No subdomains found</span>'
    ...
    if (lines.length === 40) html += `<div style="font-size:10px;color:var(--text3);margin-top:4px">Showing first 40 — more may exist</div>`
    return html
  } catch (e) { return `<span style="color:var(--flo)">${esc(String(e))}</span>` }
}
...
async function fetchDnsSecurity(domain: string): Promise<string> {
  try {
    const [spfRes, dmarcRes, dkimRes] = await Promise.allSettled([
      ...
      fetch(`https://dns.google/resolve?name=_dmarc.${encodeURIComponent(domain)}&type=TXT`).then(r => r.json()),
      fetch(`https://dns.google/resolve?name=default._domainkey.${encodeURIComponent(domain)}&type=TXT`).then(r => r.json()),
    ])

    const spfRecords = spfRes.status === 'fulfilled'
      ? ((spfRes.value as {Answer?: {data: string}[]}).Answer ?? []).filter(a => a.data.includes('v=spf1'))
      : []
    const dmarcRecords = dmarcRes.status === 'fulfilled'
      ? ((dmarcRes.value as {Answer?: {data: string}[]}).Answer ?? []).filter(a => a.data.includes('v=DMARC1'))
      : []
    const dkimRecords = dkimRes.status === 'fulfilled'
      ? ((dkimRes.value as {Answer?: {data: string}[]}).Answer ?? []).filter(a => a.data.includes('v=DKIM1'))
      : []
    ...
    return check(hasSPF,   'SPF',   hasSPF   ? spfRecords[0].data   : 'No SPF record — anyone can spoof this domain')
         + check(hasDMARC, 'DMARC', hasDMARC ? dmarcRecords[0].data : 'No DMARC policy — phishing risk')
         + check(hasDKIM,  'DKIM',  hasDKIM  ? dkimRecords[0].data  : 'No DKIM (default selector) — check selector name')
  } catch (e) { return `<span style="color:var(--flo)">${esc(String(e))}</span>` }
}
...
async function fetchEmailRep(email: string): Promise<string> {
  try {
    ...
    const rows: [string, string][] = [
      ...
      ['Suspicious', badge(d.suspicious ? 'YES' : 'NO', d.suspicious ? '#ef4444' : '#10b981')],
      ['References', String(d.references ?? 0)],
    ]
    if (d.details?.blacklisted) rows.push(['Blacklisted', badge('YES', '#ef4444')])
    if (d.details?.malicious_activity) rows.push(['Malicious activity', badge('YES', '#ef4444')])
    if (d.details?.credentials_leaked) rows.push(['Credentials leaked', badge('YES', '#f59e0b')])
    if (d.details?.spam) rows.push(['Spam source', badge('YES', '#f59e0b')])
    if (d.details?.data_breach) rows.push(['Data breach', badge('YES', '#f59e0b')])
    if (d.details?.days_since_domain_creation) rows.push(['Domain age', `${d.details.days_since_domain_creation} days`])
    if (d.details?.profiles?.length) rows.push(['Profiles', (d.details.profiles as string[]).map(esc).join(', ')])
    return table(rows)
  } catch (e) { return `<span style="color:var(--flo)">${esc(String(e))}</span>` }
}
...
async function fetchUsername(username: string): Promise<string> {
  try {
    ...
    const [ghRes, gravatarRes] = await Promise.allSettled([
      fetch(`https://api.github.com/users/${encodeURIComponent(username)}`).then(r => r.json()),
      fetch(`https://www.gravatar.com/${encodeURIComponent(username)}.json`).then(r => r.json()),
    ])

    let html = ''

    if (ghRes.status === 'fulfilled' && (ghRes.value as {login?: string}).login) {
    ...
    if (gravatarRes.status === 'fulfilled' && (gravatarRes.value as {entry?: unknown[]}).entry?.length) {
      const entry = ((gravatarRes.value as {entry: Record<string, unknown>[]}).entry)[0]
      html += `<div style="font-size:10px;font-weight:700;color:var(--accent);margin:10px 0 6px">Gravatar</div>`
      const gRows: [string, string][] = []
      if (entry.displayName) gRows.push(['Name',     esc(String(entry.displayName))])
      if (entry.currentLocation) gRows.push(['Location', esc(String(entry.currentLocation))])
      if ((entry.emails as {value?: string}[] | undefined)?.[0]?.value)
        gRows.push(['Email', esc((entry.emails as {value: string}[])[0].value)])
      if (gRows.length) html += table(gRows)
    }

    return html || '<span style="color:var(--text3)">No public profiles found</span>'
  } catch (e) { return `<span style="color:var(--flo)">${esc(String(e))}</span>` }
}
...
// ── Panel component ────────────────────────────────────────────────────────────
function Panel({ title, tag, content, loading }: { title: string; tag?: string; content: string; loading: boolean }) {
  return (
    <div style={{ background: 'var(--surf2)', border: '1px solid var(--border)', borderRadius: '10px', padding: '12px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
        <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.5px' }}>{title}</div>
        {tag && <span style={{ fontSize: '9px', fontWeight: 700, padding: '1px 5px', borderRadius: '3px', background: 'var(--surf3)', color: 'var(--text3)' }}>{tag}</span>}
      </div>
      {loading
...
export default function ReconLookup() {
  ...
  const placeholder = useCallback((key: PanelKey, msg: string) => {
  ...
  async function scan() {
    ...
    setLoading(true)
    setLoadingMap({
      rdap: true, dns: true, certs: true, geo: true,
      subdomains: true, dnssec: true, emailrep: true,
      username: true, hibp: true, vt: true, shodan: true,
    })
  ...
  const resolvedType = typeVal === 'auto' && target.trim() ? detectType(target.trim()) : typeVal
  ...
  return (
    <div>
      ...
      {/* Detected type hint */}
      {target.trim() && (
        <div style={{ fontSize: '10px', color: 'var(--text3)', marginBottom: '14px' }}>
          Detected: <span style={{ color: 'var(--accent)', fontWeight: 700 }}>{resolvedType.toUpperCase()}</span>
          {' '}— free panels will run automatically. BYOK panels require optional keys in ⚙️ Settings.
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '10px' }}>
        ...
        <Panel title="TLS Certificates"     content={panels.certs}      loading={loadingMap.certs} />
        <Panel title="IP Geolocation"       content={panels.geo}        loading={loadingMap.geo} />
        <Panel title="Subdomain Enumeration" content={panels.subdomains} loading={loadingMap.subdomains} />
        <Panel title="DNS Security (SPF / DMARC / DKIM)" content={panels.dnssec} loading={loadingMap.dnssec} />

        <Category label="👤 Identity & Email Intelligence" />
        <Panel title="Email Reputation"     content={panels.emailrep}   loading={loadingMap.emailrep} />
        <Panel title="Username OSINT"       content={panels.username}   loading={loadingMap.username} />
        <Panel title="Breach Check"         content={panels.hibp}       loading={loadingMap.hibp}   tag="BYOK" />

        <Category label="🔬 Threat Intelligence" />
        <Panel title="VirusTotal"           content={panels.vt}         loading={loadingMap.vt}     tag="BYOK" />
        <Panel title="Shodan"               content={panels.shodan}     loading={loadingMap.shodan} tag="BYOK" />

      </div>

      <p style={{ fontSize: '10px', color: 'var(--text3)', marginTop: '12px' }}>
        Free — no key needed: RDAP · DNS · crt.sh · ipapi.co · HackerTarget · emailrep.io · GitHub · dns.google (SPF/DMARC/DKIM).
        BYOK (optional): Have I Been Pwned · VirusTotal · Shodan — add in ⚙️ Settings.
      </p>
    </div>
  )
}
