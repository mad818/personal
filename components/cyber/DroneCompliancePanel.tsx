...
function ScoreBar({ score, weight }: { score: number; weight: number }) {
  const color =
    score >= 80 ? "var(--flo)" : score >= 55 ? "var(--fmd)" : "#ef4444";
  return (
...
export function DroneCompliancePanel() {
  ...
  const [opType, setOpType] =
    useState<DroneComplianceCheck["operationType"]>("commercial");
  ...
  useEffect(() => {
    ...
    if (
      ...
      OPERATION_TYPES.includes(
        operationTypeParam as DroneComplianceCheck["operationType"],
      )
    ) {
    ...
    if (nearAirportParam !== null)
      setNearApt(parseBooleanParam(nearAirportParam));
  ...
  const run = async () => {
    ...
    try {
      ...
      const message =
        caught instanceof Error ? caught.message : "Compliance check failed";
      setError(
        hasRetainedResult
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
      ...
      >
        <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
          ...
          <input
            aria-label="Drone operation city"
            style={INPUT}
            value={city}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
          ...
          <input
            aria-label="Drone operation state"
            style={INPUT}
            value={state}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
          ...
          <select
            aria-label="Drone operation type"
            style={INPUT}
            value={opType}
            onChange={(event) =>
              setOpType(
                event.target.value as DroneComplianceCheck["operationType"],
              )
            }
          >
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          >
            ...
            <input
              aria-label="Drone weight in pounds"
              style={INPUT}
              type="number"
              value={weight}
          ...
          >
            ...
            <input
              aria-label="Drone altitude in feet"
              style={INPUT}
              type="number"
              value={altitude}
        ...
        >
          ...
          >
            <input
              type="checkbox"
              checked={nightOps}
              onChange={(event) => setNightOps(event.target.checked)}
            />
            Night operations
          </label>
          <label
          ...
          >
            <input
              type="checkbox"
              checked={nearApt}
              onChange={(event) => setNearApt(event.target.checked)}
            />
            Near airport (&lt;5mi)
          </label>
        </div>
      ...
      {error ? (
        <SurfaceCallout
          ...
          title={
            hasRetainedResult
              ? "Latest compliance run failed · showing last good result"
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
          ...
          >
            <div style={{ textAlign: "center", minWidth: "56px" }}>
              ...
              <div style={{ fontSize: "9px", color: "var(--text2)" }}>
                / 100
              </div>
            </div>
            <div style={{ flex: 1 }}>
      ...
      {!result && !loading && !error ? (
        ...
        >
          Enter mission details above to run the protected FAA, state, local,
          airspace, and operational compliance check.
        </div>
      ) : null}
    </div>
  );
