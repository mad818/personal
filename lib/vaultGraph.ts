...
export function computeRelevanceDecay(
  ...
  return edges.map(e => ({
    ...e,
    weight: e.weight * (1 - 0.3 * Math.max(nodeAge[e.source] ?? 0, nodeAge[e.target] ?? 0)),
  }));
}
...
export function buildVaultGraph(
  ...
  const rawEdges  = buildAdjacency(items, texts);
...
export function runVaultLint(
  ...
  return { contradictions, staleClaims, orphanPages, gapTopics, checkedAt: now };
}
