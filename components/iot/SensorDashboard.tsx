    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: "9px", color: "var(--text3)" }}>
          <span
            style={{
              color: isWindy ? "#818cf8" : "var(--text2)",
              fontWeight: 700,
            }}
          >
            {windSpeed.toFixed(1)}
...
export default function SensorDashboard() {
  ...
  useEffect(() => {
    ...
    es.onmessage = (evt) => {
      try {
        ...
        setSensors((prev) =>
          prev.map((s) => {
            if (type === "temperature" && s.name === "Temperature")
              return { ...s, value };
            if (type === "humidity" && s.name === "Humidity")
              return { ...s, value };
  ...
  const formatValue = (s: Sensor) => {
    ...
    if (Number.isInteger(s.value) || Math.abs(s.value) >= 100)
    ...
    <div>
      ...
      >
        ...
        {sensors.map((s) => {
          ...
          const pct = Math.max(
            0,
            Math.min(100, ((s.value - s.min) / (s.max - s.min)) * 100),
          );
          ...
          return (
            ...
            >
              ...
              <div
                style={{
                  fontSize: boolVal ? "16px" : "24px",
                  fontWeight: 900,
                  color: alert ? s.color : "var(--text)",
                  marginBottom: "4px",
                  fontFamily: "monospace",
                }}
              >
