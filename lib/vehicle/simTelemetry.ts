...
const SENSOR_CONFIG: Array<
  Pick<VehicleSensorStatus, "id" | "label" | "sensorClass">
> = [
...
const CAMERA_CONFIG: Array<
  Omit<VehicleCameraFeed, "status" | "fps"> & { sensorId?: VehicleSensorId }
> = [
  {
    id: "vc1",
    label: "Forward RGB",
...
function buildEvents(
  ...
  if (missionPhase === "landing") {
    events.unshift({
      ...
      severity: "warning",
      message:
        "Emergency landing path simulated. Verify the control boundary before live bridge work.",
    });
  }
  ...
  if (batteryPercent <= 28 || failsafes.battery) {
    events.unshift({
      ...
      severity: "warning",
      message:
        "Battery approaching failsafe threshold. Treat as advisory only.",
    });
  }
  ...
  if (linkQuality <= 45 || failsafes.radio) {
    events.unshift({
      ...
      severity: "critical",
      message:
        "Link quality degraded. Future live bridge must fail safe to autopilot.",
    });
  }
...
function buildSensors(now: number) {
  return SENSOR_CONFIG.map((sensor, index) => {
    ...
    return {
      ...sensor,
      active: enabled,
      healthPercent: enabled
        ? clamp(Math.round(92 + signedNoise(now / 1000 + index, 8)), 76, 100)
        : 0,
      latencyMs: enabled
        ? clamp(
            Math.round(latencyBase + signedNoise(now / 1000 + index * 2, 4)),
            1,
            80,
          )
        : 0,
...
function buildCameras(now: number) {
  return CAMERA_CONFIG.map((camera, index) => {
    const active = camera.sensorId
      ? simulation.sensorEnabled[camera.sensorId]
      : true;
    const status: VehicleCameraFeed["status"] = !simulation.companionConnected
      ? "offline"
      : active
        ? "active"
        : "standby";
    ...
    return {
      ...
      fps:
        status === "active"
          ? clamp(
              Math.round(24 + signedNoise(now / 1000 + index * 3, 6)),
              18,
              36,
            )
          : 0,
...
function buildPipeline(now: number, companionConnected: boolean) {
  return PIPELINE_NAMES.map((name, index) => ({
    ...
    latencyMs: companionConnected
      ? clamp(
          Math.round(4 + index * 4 + signedNoise(now / 1000 + index, 3)),
          2,
          40,
        )
      : 0,
...
function buildDetections(
  ...
  return {
    people: clamp(
      Math.round(2 + signedNoise(now / 1000, 2) + activityBoost),
      0,
      6,
    ),
    vehicles: clamp(
      Math.round(1 + signedNoise(now / 1000 + 1, 2) + activityBoost),
      0,
      5,
    ),
...
function buildFrame(
  previous: VehicleTelemetryFrame | null,
  now: number,
): VehicleTelemetryFrame {
  const mode: VehicleFlightMode =
    emergencyUntil > now ? "LAND" : (previous?.heartbeat.mode ?? "LOITER");
  ...
  const headingDeg = round(
    ((previous?.position.headingDeg ?? 247) +
      signedNoise(step + 5, mode === "LOITER" ? 4 : 9) +
      360) %
      360,
    1,
  );
  ...
  const metersPerDegreeLon =
    ...
    Math.max(
      0.3,
      Math.cos((previous?.position.lat ?? BASE_LAT) * (Math.PI / 180)),
    );
  const lat = round(
    (previous?.position.lat ?? BASE_LAT) +
      (Math.cos(headingRad) * distanceM) / metersPerDegreeLat,
    5,
  );
  const lon = round(
    (previous?.position.lon ?? BASE_LON) +
      (Math.sin(headingRad) * distanceM) / metersPerDegreeLon,
    5,
  );
  ...
  const batteryPercent = clamp(
    round(
      (previous?.battery.percent ?? 91) -
        0.12 -
        Math.max(0, groundSpeedMps - 4) * 0.01,
      1,
    ),
    14,
    100,
  ...
  const currentA = round(
    clamp(9 + groundSpeedMps * 1.4 + signedNoise(step + 9, 1.8), 5, 28),
    1,
  );
  ...
  return {
    ...
    heartbeat: {
      ...
      mode,
      linkState:
        linkQuality <= 35 ? "lost" : linkQuality <= 55 ? "degraded" : "online",
    ...
    position: {
      ...
      groundSpeedMps,
      climbRateMps: round(
        mode === "LAND" ? -0.8 : signedNoise(step + 17, 0.7),
        2,
      ),
      headingDeg,
      fixType: simulation.sensorEnabled.gps ? "3D" : "2D",
      satellites: simulation.sensorEnabled.gps
        ? clamp(Math.round(12 + signedNoise(step + 19, 2)), 8, 16)
        : 4,
      hdop: simulation.sensorEnabled.gps
        ? round(clamp(0.9 + signedNoise(step + 21, 0.3), 0.7, 1.9), 2)
        : 2.8,
    },
    battery: {
    ...
    link: {
      ...
      downlinkKbps: clamp(
        Math.round(164 + signedNoise(step + 25, 28)),
        72,
        260,
      ),
    },
    mission: {
      ...
      currentStep:
        simulation.waypointCount === 0
          ? 0
          : (step % Math.max(1, simulation.waypointCount)) + 1,
      totalSteps: simulation.waypointCount,
      homeDistanceM: clamp(
        Math.round(84 + signedNoise(step + 27, 26)),
        18,
        220,
      ),
    },
    ...
    motors: [
      {
        ...
        health: "ok",
        rpm: clamp(
          Math.round(1400 + groundSpeedMps * 110 + signedNoise(step + 29, 90)),
          900,
          3200,
        ),
      },
      {
        ...
        health: "ok",
        rpm: clamp(
          Math.round(1420 + groundSpeedMps * 108 + signedNoise(step + 31, 90)),
          900,
          3200,
        ),
      },
      {
        ...
        health: "ok",
        rpm: clamp(
          Math.round(1380 + groundSpeedMps * 112 + signedNoise(step + 33, 90)),
          900,
          3200,
        ),
      },
      {
        ...
        rpm: clamp(
          Math.round(1410 + groundSpeedMps * 106 + signedNoise(step + 35, 90)),
          900,
          3200,
        ),
      },
    ...
    companion: {
      ...
      cpuTempC: simulation.companionConnected
        ? clamp(Math.round(54 + signedNoise(step + 37, 8)), 40, 82)
        : 0,
      gpuTempC: simulation.companionConnected
        ? clamp(Math.round(63 + signedNoise(step + 39, 10)), 46, 88)
        : 0,
      aiModel:
        simulation.companionRoute === "local"
          ? "edge-detect-qwen"
          : "remote-analysis-proxy",
      inferenceMs: simulation.companionConnected
        ? clamp(Math.round(18 + signedNoise(step + 41, 7)), 10, 38)
        : 0,
    },
    detections,
    fusionConfidencePercent,
    pipeline,
    recentEvents: buildEvents(
      batteryPercent,
      linkQuality,
      missionPhase,
      failsafes,
...
export function makeSimulatedTelemetryFrame(
  input: SimulatedTelemetryFrameInput,
): VehicleTelemetryFrame {
  ...
  try {
    ...
    const linkQualityPercent =
      input.linkQualityPercent ?? frame.link.qualityPercent;
    const failsafes: VehicleFailsafeStatus = {
      ...frame.failsafes,
    ...
    return {
        ...
        linkState:
          linkQualityPercent <= 35
            ...
            : linkQualityPercent <= 55
              ...
              : "online",
        health:
          failsafes.battery || failsafes.radio
            ? "warning"
            : frame.heartbeat.health,
      },
      battery: {
        ...frame.battery,
        percent: batteryPercent,
      ...
      failsafes,
      recentEvents: event
        ? [event, ...frame.recentEvents].slice(0, 3)
        : frame.recentEvents,
    };
  } finally {
    step = originalStep;
...
function buildSnapshot(): VehicleTelemetrySnapshot {
  ...
  const activeFrame =
    resolvedReplayIndex === null ? latestFrame : history[resolvedReplayIndex];
  ...
  return {
    ...
    replayOffsetSeconds:
      resolvedReplayIndex === null
        ? 0
        : history.length - 1 - resolvedReplayIndex,
...
function advanceSimulation(now: number) {
  ...
  if (
    emergencyUntil <= now &&
    latestFrame.heartbeat.mode === "LAND" &&
    simulation.waypointCount > 0
  ) {
    latestFrame = {
      ...latestFrame,
...
export function setVehicleSimSensorEnabled(
  id: VehicleSensorId,
  enabled: boolean,
) {
  updateSimulation(() => {
