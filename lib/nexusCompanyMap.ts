...
export const COMPANY_SKILL_SOURCES: readonly CompanySkillSource[] = [
  ...
  },
  {
    ...
    kind: "reference",
    posture: "adapted",
    purpose: "Multi-candidate model evaluation with explicit rubrics, judge variance, cost evidence, and operator selection.",
    codexPath: "Apply the evaluation pattern through Nexus's existing AI and eval gates; do not import its direct-provider Python runtime.",
    chatgptPath: "Use a bounded comparison brief with declared criteria, uncertainty, and cost; keep the final choice with the operator.",
  },
  {
    id: "pliny-leakhub",
    ...
    kind: "reference",
    posture: "adapted",
    purpose: "Consensus, provenance, and verification-state cues for defensive prompt-transparency evidence.",
    codexPath: "Use the Apache-2.0 verification pattern in defensive evidence reviews; never solicit, publish, or reward stolen prompts.",
    chatgptPath: "Require source provenance and independent corroboration before treating prompt-transparency claims as verified.",
  },
  {
    ...
    kind: "reference",
    posture: "review_first",
    purpose: "A broad taxonomy for local text normalization, encoding recognition, and defensive steganography awareness.",
    codexPath: "Translate only reviewed safe transforms into Nexus-owned code; AGPL source, hidden-message creation, and provider bypasses stay external.",
    chatgptPath: "Use the taxonomy for explanation or defensive decoding guidance; do not imply the external transform runtime is installed.",
  },
  {
    ...
    kind: "reference",
    posture: "adapted",
    purpose: "Plan, draft, critique, and revise creative work as explicit stages instead of one opaque generation pass.",
    codexPath: "Reuse the Apache-2.0 workflow idea through lib/ai.ts and existing review surfaces; do not add its direct provider clients.",
    chatgptPath: "Carry a staged story brief with explicit human approval between outline, draft, critique, and revision.",
  },
  {
    id: "pliny-ourobopus",
    ...
    kind: "reference",
    posture: "adapted",
    purpose: "Generate, inspect, measure, and propose improvements while keeping every change review-gated.",
    codexPath: "Map the Apache-2.0 loop onto Nexus evals, lessons, and proposed edits; no autonomous self-modification or direct provider calls.",
    chatgptPath: "Return improvement proposals with evidence and rollback criteria; never claim that a chat changed its own runtime.",
  },
  {
    id: "marketing-skills",
...
export const NEXUS_COMPANY_DEPARTMENTS: readonly NexusCompanyDepartment[] = [
  {
    ...
    sourceIds: ["nexus-native", "superpowers", "mattpocock-skills", "davidondrej-skills", "pliny-autotemp", "pliny-ourobopus"],
  ...
  },
  {
    ...
    sourceIds: ["superpowers", "mattpocock-skills", "mattpocock-deep-modules-pr", "davidondrej-skills", "context7", "anthropic-skills", "graphify", "claude-mem", "pliny-autotemp", "pliny-ourobopus"],
  ...
  },
  {
    ...
    sourceIds: ["last30days-skill", "davidondrej-skills", "graphify", "context7", "claude-mem", "nexus-native", "pliny-leakhub", "pliny-parseltongue"],
  ...
  },
  {
    ...
    sourceIds: ["last30days-skill", "frontend-slides", "marketing-skills", "social-media-skills", "anthropic-skills", "pliny-autostorygen"],
  ...
  },
  {
    ...
    sourceIds: ["claude-business-plugins", "anthropic-skills", "nexus-native", "pliny-leakhub", "pliny-parseltongue"],
