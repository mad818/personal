...
export default function WorldEventMap() {
  ...
  const gdeltPins = useMemo(() => {
    return gdeltEvents
      ...
      .map((ev: Record<string, unknown>, i: number) => {
        ...
        const [x, y] = latLonToXY(lat, lon);
        const title =
          typeof ev.title === "string" ? ev.title : "Conflict event";
  ...
  const totalEvents =
    earthquakePins.length + gdeltPins.length + (weatherPin ? 1 : 0);
          ...
          {tooltip && (
            <motion.div
              ...
              style={{
                ...
                border: `1px solid ${
                  tooltip.type === "earthquake"
                    ? CHART.red
                    : tooltip.type === "weather"
                      ? CHART.sky
                      : CHART.orange
                }66`,
                borderRadius: "6px",
