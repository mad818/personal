...
function TelemetryHUD() {
  const { prices, fg, worldRisk, cves, articles, agentStats, agentRuntime } =
    useStore(
      (s) => ({
        prices: s.prices,
        fg: s.signals?.fg,
  ...
  const [evalLatest, setEvalLatest] = useState<{
    score: number;
    minScore?: number;
    stale?: boolean;
    failures?: number;
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (d?.latest && typeof d.latest.score === "number") {
          setEvalLatest({
            ...
            failures:
              (d.failures?.checks?.length ?? 0) +
              (d.failures?.categories?.length ?? 0),
            failureStreak: Number(d.runner?.failureStreak ?? 0),
            effectiveCooldownMin:
              typeof d.runner?.effectiveCooldownMin === "number"
                ? d.runner.effectiveCooldownMin
                : undefined,
            fetchedAt: Date.now(),
          });
        }
    ...
    const firstRun = window.setTimeout(() => {
      void load();
    }, staggerDelayMs("telemetry-hud-eval"));
    const t = window.setInterval(() => {
      void load();
    }, RUNTIME_POLL_MS.telemetryEval);
    const onVisible = () => {
    ...
    >
      ...
      {fg ? (
        <TickerItem
          ...
          subColor={fgColor(
            typeof fg.value === "number" ? fg.value : Number(fg.value),
          )}
          color={fgColor(
            typeof fg.value === "number" ? fg.value : Number(fg.value),
          )}
      ...
      <TickerItem
        ...
        color={
          agentRuntime.status === "verified"
            ? "#00FF66"
            : agentRuntime.status === "degraded"
              ? "#f59e0b"
      ...
      {evalLatest && (
        <>
          {(() => {
            const evalGrade = gradeFromEvalScore(evalLatest.score, {
              stale: evalLatest.stale,
            });
            const evalColor = evalLatest.stale
              ? "#f59e0b"
              : evalGradeColor(evalGrade);
            const evalSubColor = evalLatest.stale
              ? "#f59e0b"
              : Number(evalLatest.failures ?? 0) > 0
                ? "#ef4444"
                : evalColor;
            return (
              <>
                ...
                <TickerItem
                  ...
                  sub={
                    evalLatest.stale
                      ...
                      : typeof evalLatest.minScore === "number"
                        ? `MIN ${evalLatest.minScore}${evalLatest.failures ? ` · ${evalLatest.failures}x` : ""}`
                        : undefined
                  }
                  ...
                  dot
                  title={
                    evalLatest.fetchedAt
                      ? `Updated ${new Date(evalLatest.fetchedAt).toLocaleTimeString()}`
                      : undefined
                  }
                />
              </>
          ...
          {Number(evalLatest.failureStreak ?? 0) > 0 && (
            <>
              ...
              <TickerItem
                ...
                sub={
                  typeof evalLatest.effectiveCooldownMin === "number"
                    ? `${evalLatest.effectiveCooldownMin}m`
                    : undefined
                }
                subColor="#f59e0b"
                color="#f59e0b"
