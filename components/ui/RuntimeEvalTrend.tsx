...
import { parseRuntimeEvalPayload, parseStatusPayload, type RuntimeEvalPayload } from '@/lib/runtimeTypes'
...
export default function RuntimeEvalTrend() {
  ...
  const refresh = useCallback(async (): Promise<RuntimeEvalPayload> => {
    const raw = await fetchJsonCached('runtime-eval:limit=24', async () => {
    ...
    const timer = window.setInterval(() => {
      void (async () => {
        ...
        try {
          ...
          const nextEligibleAt = snapshot?.runner?.nextEligibleAt ? new Date(snapshot.runner.nextEligibleAt).getTime() : 0
  ...
  const spark = useMemo(() => {
    const arr = (data?.history ?? []).map((h) => Math.max(0, Math.min(100, Number(h.score || 0))))
    if (!arr.length) return ''
    const chars = '▁▂▃▄▅▆▇█'
    return arr
      .map((v) => chars[Math.min(chars.length - 1, Math.floor((v / 100) * (chars.length - 1)))])
      .join('')
  }, [data?.history])
  ...
  const failureCount = (data?.failures?.checks?.length ?? 0) + (data?.failures?.categories?.length ?? 0)
  ...
  const runNow = async (force = false) => {
    ...
    try {
      ...
      const d = await r.json() as { ok?: boolean; skipped?: boolean; reason?: string; output?: string }
      if (!r.ok || !d.ok) {
  ...
  return (
    <div style={{
      ...
      {loading ? (
        <div style={{ fontSize: 12, color: 'var(--text3)' }}>Loading runtime metrics…</div>
      ) : !latest ? (
        ...
        <>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            ...
            {ageMin !== null && (
              <span style={{ fontSize: 11, color: stale ? '#f59e0b' : 'var(--text3)' }}>
            ...
            <span style={{
              ...
              {pass ? 'PASS' : `DEGRADED${failureCount > 0 ? ` (${failureCount})` : ''}`}
            </span>
          </div>
          {lastUpdatedAt && (
            <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: -4, marginBottom: 6 }}>
          ...
          {categoryEntries.length > 0 && (
            <div style={{ marginTop: 8, display: 'grid', gap: 6 }}>
              {categoryEntries.map(([name, value]) => {
                ...
                return (
                  <div key={name} style={{ display: 'grid', gridTemplateColumns: '88px 1fr 34px', gap: 8, alignItems: 'center' }}>
                    <span style={{ fontSize: 10, color: 'var(--text3)', textTransform: 'capitalize' }}>{name}</span>
                    <div style={{ height: 6, borderRadius: 999, background: '#1A2040', overflow: 'hidden' }}>
                      <div
                        style={{
                          width: `${score}%`,
                          height: '100%',
                    ...
                    <span style={{ fontSize: 10, color: 'var(--text3)', textAlign: 'right' }}>{Math.round(score)}</span>
                  </div>
                )
              })}
            </div>
          )}
          {(data?.failures?.checks?.length || data?.failures?.categories?.length) ? (
            <div style={{ marginTop: 8, padding: '6px 8px', border: '1px solid #3a2530', borderRadius: 6, background: 'rgba(239,68,68,0.06)' }}>
              <div style={{ fontSize: 10, color: '#f59e0b', marginBottom: 4, fontWeight: 700 }}>
                Why degraded
              </div>
              {(data.failures?.checks ?? []).map((f, i) => (
                <div key={`c-${i}`} style={{ fontSize: 10, color: 'var(--text3)' }}>
                  check: {f.name ?? 'unknown'} ({f.category ?? 'unknown'})
                </div>
              ))}
              {(data.failures?.categories ?? []).map((f, i) => (
                <div key={`k-${i}`} style={{ fontSize: 10, color: 'var(--text3)' }}>
                  category: {f.name ?? 'unknown'} score {Math.round(Number(f.score ?? 0))} / min {Math.round(Number(f.threshold ?? 0))}
                </div>
              ))}
            </div>
          ) : null}
          ...
          {data?.runner?.nextEligibleAt && (
            <div style={{ fontSize: 10, color: 'var(--text3)', marginTop: 2 }}>
              ...
              {typeof data.runner.effectiveCooldownMin === 'number' ? ` (effective ${data.runner.effectiveCooldownMin}m)` : ''}
              {typeof data.runner.failureStreak === 'number' && data.runner.failureStreak > 0 ? ` · backoff x${2 ** data.runner.failureStreak}` : ''}
              {' · '}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8 }}>
            <button
              ...
              style={{
                ...
                background: running ? 'rgba(26,32,64,0.6)' : 'rgba(13,18,32,0.96)',
                color: '#7ba7d4',
                padding: '5px 9px',
                fontSize: 10,
                fontWeight: 700,
            ...
            <button
              ...
              style={{
                ...
                background: running ? 'rgba(26,32,64,0.6)' : 'rgba(36,14,26,0.96)',
                color: '#f59e0b',
                padding: '5px 9px',
                fontSize: 10,
                fontWeight: 700,
            ...
            {runMsg && <span style={{ fontSize: 10, color: 'var(--text3)' }}>{runMsg}</span>}
          </div>
        </>
      )}
        ...
        >
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8 }}>
              <div style={{ fontSize: 12, fontWeight: 800, color: '#7ba7d4', letterSpacing: '.04em' }}>Status Diagnostics</div>
              <button
                type="button"
                onClick={() => { void refreshStatusDrawer() }}
                style={{
                  marginLeft: 8,
                  borderRadius: 8,
                  border: '1px solid #2a3a6b',
            ...
            >
              {statusLoading ? 'Loading status…' : JSON.stringify(statusPayload, null, 2)}
            </pre>
          </div>
        </div>
      )}
