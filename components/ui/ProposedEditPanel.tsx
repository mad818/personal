...
function DiffView({ oldStr, newStr }: { oldStr: string; newStr: string }) {
  ...
  function Lines({ lines, color, prefix }: { lines: string[]; color: string; prefix: string }) {
      ...
      <div>
        {shown.map((ln, i) => (
          <div key={i} style={{
            ...
            <span style={{ color: `${color}66`, flexShrink: 0, userSelect: 'none' }}>{prefix}</span>
            <span>{ln || ' '}</span>
          </div>
        ))}
        {!showFull && lines.length > MAX_PREVIEW && (
          <button
            ...
            style={{
              fontSize: '8px', color: '#4a5568', background: 'none',
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '3px 6px', background: '#0a0e1a', borderBottom: '1px solid #1A2040' }}>
        <span style={{ fontSize: '8px', fontFamily: 'monospace', color: '#304060' }}>BEFORE</span>
      </div>
      <Lines lines={oldLines} color="#ef4444" prefix="−" />
      <div style={{ padding: '3px 6px', background: '#0a0e1a', borderTop: '1px solid #1A2040', borderBottom: '1px solid #1A2040' }}>
        <span style={{ fontSize: '8px', fontFamily: 'monospace', color: '#304060' }}>AFTER</span>
      </div>
...
// ── Single edit card ──────────────────────────────────────────────────────────
function EditCard({ edit, onApprove, onReject, busy }: {
  edit:      PendingEdit
  onApprove: () => void
  onReject:  () => void
  busy:      boolean
  ...
  return (
    <div style={{
      ...
      >
        <span style={{
          fontSize: '9px', fontFamily: "'VT323', monospace",
          color: '#00DDFF', letterSpacing: '.5px', flex: 1, minWidth: 0,
          overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
        }}>
          ✏️ {edit.path}
        </span>
        <RiskBadge risk={edit.risk} />
        <span style={{ fontSize: '9px', color: '#304060' }}>{expanded ? '▲' : '▼'}</span>
      </div>
      ...
      {expanded && (
        <div style={{ padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {/* Reason */}
          <div style={{
            fontSize: '9px', color: '#6875a0',
            fontFamily: 'monospace', lineHeight: 1.5,
            padding: '4px 8px', background: 'rgba(79,110,247,.05)',
            borderRadius: '4px', borderLeft: '2px solid #4f6ef744',
          }}>
            {edit.reason}
          </div>
          ...
          <div style={{ display: 'flex', gap: '6px', justifyContent: 'flex-end' }}>
            ...
            <button
              ...
              style={{
                fontSize: '9px', fontFamily: "'VT323', monospace",
                padding: '3px 12px', borderRadius: '4px',
                background: busy ? 'rgba(0,255,102,.04)' : 'rgba(0,255,102,.12)',
                border: '1px solid #00FF6655',
                color: '#00FF66', cursor: busy ? 'not-allowed' : 'pointer',
                letterSpacing: '1px',
              }}
            >
...
export default function ProposedEditPanel() {
  ...
  async function handleApprove(edit: PendingEdit) {
    ...
    try {
      ...
      const success = r.ok && !d.result?.startsWith('Patch failed') && !d.result?.startsWith('Blocked')

      addChangeEntry({
        path:         edit.path,
        agent:        edit.agentId,
        summary:      success ? `Applied: ${edit.reason.slice(0, 80)}` : `Failed: ${d.result?.slice(0, 80)}`,
    ...
    <div style={{
      ...
      {open && (
        <div style={{
          ...
          borderBottom: 'none',
          boxShadow: '0 -4px 32px rgba(0,0,0,.6), 0 0 24px rgba(0,255,102,.06)',
          ...
          <div style={{
            ...
            <span style={{
              ...
              color: '#f59e0b', letterSpacing: '2px', flex: 1,
            }}>
              PROPOSED EDITS
            </span>
            <span style={{
      ...
      >
        ...
        <span style={{
          ...
          {pendingEdits.length} EDIT{pendingEdits.length !== 1 ? 'S' : ''} AWAITING REVIEW
        </span>
        <span style={{ fontSize: '8px', color: '#304060' }}>{open ? '▼' : '▲'}</span>
      </button>
    </div>
  )
}
