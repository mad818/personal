...
function TagEditor({ articleId, tags }: { articleId: string; tags: string[] }) {
  ...
  return (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', alignItems: 'center', marginTop: '8px' }}>
      {tags.map((t) => (
        <span key={t} style={{
      ...
      <button
        ...
        style={{
          ...
          borderRadius: 'var(--radius-pill)', color: 'var(--text3)',
          fontSize: '10px', padding: '2px 7px', cursor: 'pointer',
...
export default function SavedArticles() {
  ...
  const [sortOrder,  setSortOrder]  = useState<'newest' | 'oldest' | 'mostTagged'>('newest')
  ...
  const visible = useMemo(() => {
    ...
    let result = savedArticles.filter((a) => {
      ...
      if (q) {
        const haystack = [a.title, a.desc ?? '', ...(a.tags ?? [])].join(' ').toLowerCase()
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <div style={{ display: 'flex', gap: '6px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <select
          ...
          style={{
            ...
            borderRadius: '7px', color: 'var(--text2)', fontSize: '11px',
        ...
        {/* Count badge */}
        <span style={{
          fontSize: '10px', color: 'var(--text3)', whiteSpace: 'nowrap',
          background: 'var(--surf3)', padding: '4px 8px', borderRadius: '20px',
        }}>
      ...
      {allTags.length > 0 && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', alignItems: 'center' }}>
          <span style={{ fontSize: '10px', color: 'var(--text3)', marginRight: '2px', flexShrink: 0 }}>Filter:</span>
