...
| **TradingView embed widgets** (ticker, advanced chart, heatmaps, etc.)                              | **Partial → now extended** | Full suite still only in `archive/components/tradingview/`. **Assimilated:** MARKETS tab **📺 CHARTS** (`/alpha?view=charts`) uses **ticker tape + BTC advanced chart** (`components/alpha/TradingViewMarkets.tsx`) through the fixed opaque-origin `/embeds/tradingview` sandbox. Optional: port screener, heatmap, etc. later. |
...
1. **More TradingView widgets** — adapt only useful candidates from `archive/components/tradingview/` into `components/alpha/`; each must extend the fixed sandbox kind/config inventory and its focused security proof rather than execute provider scripts in Nexus.
