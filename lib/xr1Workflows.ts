...
export interface MarketReviewCoachSuggestions {
  whatToRepeat: string;
  whatToAvoid: string;
  nextRule: string;
}
...
export function buildCompiledPageHref(
  ...
  params.set("focus", "vault-compiled-pages");
  params.set("pageId", page.id);
  if (page.workflowId) {
    params.set("workflowId", page.workflowId);
  }
...
export function buildMarketReviewMarkdown(draft: MarketReviewDraft) {
  const sections: Array<[string, string]> = [
    ...
    ["What to repeat", draft.whatToRepeat],
    ["What to avoid", draft.whatToAvoid],
    ["Next rule", draft.nextRule],
  ];
...
export function parseMarketReviewMarkdown(content: string): MarketReviewDraft {
  ...
  return {
    ...
    whatToRepeat: sections.get("what to repeat") ?? "",
    whatToAvoid: sections.get("what to avoid") ?? "",
    nextRule: sections.get("next rule") ?? "",
  };
}

export function buildMarketReviewCoachSuggestions(
  draft: Pick<
    MarketReviewDraft,
    "asset" | "setup" | "invalidation" | "result" | "emotionalPosture"
  >,
  ...
  const resultValue = normalizeDraftValue(draft.result) || priorDraft.result;
  const postureValue =
    normalizeDraftValue(draft.emotionalPosture) || priorDraft.emotionalPosture;

  const whatToRepeat = cleanInline(
    setupValue
      ? `Repeat the setup discipline that kept ${assetLabel} structured: ${cleanInline(setupValue, 104)}`
      : priorDraft.whatToRepeat ||
          `Repeat the habit of writing the setup before adding ${assetLabel} risk.`,
    160,
  );

  const whatToAvoid = cleanInline(
    invalidationValue
      ? `Avoid overriding the invalidation once ${trimSentenceEnding(cleanInline(invalidationValue, 104))} triggers.`
      : resultValue
  ...
  const nextRule = cleanInline(
    postureValue
      ? `When the posture feels ${cleanInline(postureValue, 48)}, restate the invalidation before changing size or thesis.`
      : invalidationValue
...
export function seedMarketReviewCoachFields(
  draft: MarketReviewDraft,
  suggestions: MarketReviewCoachSuggestions,
): MarketReviewDraft {
  return {
    ...draft,
    whatToRepeat: draft.whatToRepeat.trim() || suggestions.whatToRepeat,
    whatToAvoid: draft.whatToAvoid.trim() || suggestions.whatToAvoid,
    nextRule: draft.nextRule.trim() || suggestions.nextRule,
  };
}
