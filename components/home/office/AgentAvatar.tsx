...
import {
  AGENTS, DESK_DECO,
  IDLE_ANIM, IDLE_SPRITE_ANIM, TOOL_ICON,
} from './constants'
import type { AvatarProps } from './types'

export function AgentAvatar({
  id, active, routing, dispatched, dispatch, activeTool, isReasoning,
}: AvatarProps) {
  ...
  const innerAnim = dispatched || active
    ...
    <div style={{
      ...
      {dispatch && (
        <div style={{
          ...
          background: 'var(--surf)', border: `1px solid ${cfg.color}66`,
          borderRadius: '8px', padding: '5px 10px',
          fontSize: '9px', fontWeight: 700, color: cfg.color,
          whiteSpace: 'nowrap', zIndex: 20,
          animation: 'bubbleUp .2s ease', boxShadow: '0 4px 12px rgba(0,0,0,.5)',
        }}>
          {dispatch}
      ...
      {activeTool && active && (
        <div style={{
          ...
          transform: 'translateX(-50%)',
          zIndex: 25, animation: 'bubbleUp .2s ease', pointerEvents: 'none',
        }}>
          {isReasoning ? (
            // R1 thought-cloud — dashed teal border, dots prefix
            <div style={{
              background: 'rgba(20,184,166,0.12)',
              border: '1px dashed #14b8a688', borderRadius: '10px',
              padding: '4px 8px', fontSize: '7px',
              fontFamily: "'VT323', monospace", color: '#14b8a6',
            ...
            <div style={{
              ...
              color: cfg.color, letterSpacing: '1px', whiteSpace: 'nowrap',
              boxShadow: `0 0 8px ${cfg.color}28`,
            }}>
              {TOOL_ICON[activeTool] ?? '⚙'} {activeTool.replace(/_/g, ' ').toUpperCase()}
      ...
      {isLive && (
        <div style={{
          fontSize: '6px', fontFamily: "'VT323', monospace", letterSpacing: '2px',
          color: cfg.color + 'cc', marginBottom: '1px',
          animation: 'statusPip 1.2s ease-in-out infinite',
        }}>
          {routing ? '◉ ROUTING' : dispatched ? '◉ TASKED' : active ? '◉ WORKING' : ''}
        </div>
      )}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '3px', marginBottom: '2px' }}>
        ...
        <span style={{
          fontSize: '7px', fontWeight: 900, letterSpacing: '.1em', textTransform: 'uppercase',
        ...
        <span style={{
          ...
          animation: isLive ? 'statusPip 1.5s ease-in-out infinite' : 'none',
      ...
      <div style={{
        ...
        {isLive && (
          <div style={{
            ...
            pointerEvents: 'none', animation: 'ambientGlow 2s ease-in-out infinite',
          }} />
        )}
      ...
      </div>{/* end character-group */}

      {/* ── Status + confidence bar ── */}
      <div style={{ marginTop: '3px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1px' }}>
        <div style={{ fontSize: '7px', color: statusColor, fontWeight: 700 }}>{statusText}</div>
        ...
        {agentStats && (
          <div style={{
            ...
            <div style={{
              height: '100%', width: `${agentStats.lastConfidence}%`,
              background: agentStats.lastConfidence >= 80 ? '#00FF66'
                : agentStats.lastConfidence >= 50 ? '#00DDFF' : '#f59e0b',
