...
function generateVessels(
  ...
  for (let i = 0; i < count; i++) {
    ...
    vessels.push({
      ...
      ship_type:
        SHIP_TYPES[
          Math.floor(rng3 * 7 * SHIP_TYPES.length) % SHIP_TYPES.length
        ],
      destination:
        DESTINATIONS[
          Math.floor(rng4 * 3 * DESTINATIONS.length) % DESTINATIONS.length
        ],
  ...
  try {
    ...
    return NextResponse.json({
      ...
      meta: {
        ...
        source: hasAISKey ? "demo_with_aisstream_available" : "demo",
        note: hasAISKey
