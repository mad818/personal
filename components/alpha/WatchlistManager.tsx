...
export default function WatchlistManager() {
  ...
  const addCoin = (id: string) => {
    ...
    if (!clean || watchlist.includes(clean) || DEFAULT_COINS.includes(clean))
      return;
  ...
  if (!open) {
    return (
      ...
      >
        <span>⚙</span> Watchlist{" "}
        {allCustom.length > 0 ? `(+${allCustom.length})` : ""}
      </button>
    );
  }
  ...
  return (
    ...
    >
        ...
        <div style={{ marginBottom: "10px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
            {allCustom.map((id) => (
              ...
              >
                {id} <span style={{ fontSize: "9px", opacity: 0.7 }}>✕</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "5px" }}>
          {SUGGESTIONS.filter(
            (s) => !watchlist.includes(s.id) && !DEFAULT_COINS.includes(s.id),
          ).map((s) => (
            <button
              key={s.id}
      ...
      >
        Use CoinGecko IDs (lowercase, hyphenated). New coins appear in Price
        Overview after the next refresh.
      </div>
    </div>
  );
}
