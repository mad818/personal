...
function SummaryBar({ markets }: { markets: Market[] }) {
  ...
  const extreme   = markets.filter((m) => m.probability >= 80 || m.probability <= 20).length
  const leaning   = markets.filter((m) => (m.probability >= 65 && m.probability < 80) || (m.probability > 20 && m.probability <= 35)).length
  ...
  const chip = (label: string, val: string | number, color: string) => (
    <div style={{
      ...
      borderRadius: '8px', padding: '7px 12px', minWidth: '70px',
    }}>
      <span style={{ fontSize: '16px', fontWeight: 900, color }}>{val}</span>
      <span style={{ fontSize: '9px', color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '.06em', marginTop: '2px' }}>{label}</span>
    </div>
  )
...
export default function PolymarketFeed() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMarkets(raw.slice(0, 50).map((e) => {
        const eRaw = e as Record<string, unknown>
        const mArr = eRaw.markets as Array<Record<string, unknown>> | undefined
        let prob = 50
        try {
          const prices = mArr?.[0]?.outcomePrices
          if (typeof prices === 'string') {
            prob = Math.round(parseFloat((JSON.parse(prices) as string[])[0] ?? '0.5') * 100)
          }
        } catch { /* silent */ }
        return {
          id:          String(eRaw.id ?? ''),
    ...
    <div>
      ...
      {markets.length > 0 && (
        <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '10px', alignItems: 'center' }}>
          ...
          <input
            ...
            style={{
              marginLeft: 'auto', height: '26px', padding: '0 10px',
              borderRadius: '6px', border: '1px solid var(--border2)',
              background: 'var(--surf2)', color: 'var(--text)',
              fontSize: '11px', outline: 'none', minWidth: '160px',
            }}
          />
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '9px' }}>
        {sorted.map((m) => {
          ...
          const closingColor = days === null ? 'var(--text3)'
            : days <= 3  ? '#ef4444'
            : days <= 14 ? '#f59e0b'
            : 'var(--text3)'
          ...
          return (
            <div key={m.id} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                <span style={{ fontSize: '10px', color: 'var(--text3)', fontWeight: 600 }}>{m.category}</span>
                <span style={{ fontSize: '10px', fontWeight: 700, color: bracket.color }}>{bracket.label}</span>
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '5px' }}>
                {m.volume > 0 && (
                  <span style={{ fontSize: '10px', color: 'var(--text3)' }}>Vol {fmtVol(m.volume)}</span>
                )}
                {days !== null && (
                  <span style={{ fontSize: '10px', color: closingColor, fontWeight: days <= 14 ? 700 : 400 }}>
