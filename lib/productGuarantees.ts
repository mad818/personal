 ...
 * Nexus Prime is free software; the app must never bill end users.
 */
...
export const NEXUS_FREE_USE_DESCRIPTION =
  'Nexus Prime is free open-source software (MIT). This app does not charge you or sell subscriptions. Optional API keys are bring-your-own; any cost is only between you and that provider — not Nexus.'
...
export function assertNexusDoesNotChargeUsers(): void {
  if (NEXUS_APP_CHARGES_END_USERS !== false) {
    throw new Error(
      'Invariant violated: NEXUS_APP_CHARGES_END_USERS must stay false — Nexus Prime must not bill end users.',
    )
  }
}
...
export const NEXUS_AGENT_NO_BILLING_RULE =
  'PRODUCT INVARIANT: Nexus Prime is free open-source software (MIT). Never add or propose in-app billing, paywalls, subscriptions, in-app purchases, or payment processing for Nexus. Optional API keys are bring-your-own; any cost is only between the user and that third party — not Nexus.'
