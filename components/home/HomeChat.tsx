...
const QUICK_CHIPS = [
  {
    label: "📊 Market Pulse",
    prompt:
      "Search for the latest crypto and macro market news right now and give me a sharp pulse.",
  },
  {
    label: "⚡ Top Priority Today",
    prompt:
      "Based on my goals, what is my single highest-leverage action today? Be specific.",
  },
  {
    label: "🔍 Hidden Opportunity",
    prompt:
      "Search for a market or business opportunity most people are missing right now.",
  },
  {
    label: "📝 Research Note",
    prompt:
      "Search for the top 3 AI agent tools in 2026 and save a summary to research-notes.md.",
  },
  {
    label: "🌐 Intel Brief",
    prompt:
      "Run a full intelligence briefing — search markets, world events, and key risks. Save to intel-brief.md.",
  },
];
...
function ModeBadge({
  ...
  return (
    ...
    >
      ...
      {isLocal && hasDrafts && (
        <span style={{ opacity: 0.7 }}>• pending drafts queued</span>
      )}
      {isLocal && (
        <button
          onClick={onResetMode}
  ...
  if (step.type === "tool_call") {
    return (
      ...
      >
        ...
        {expanded && (
          <pre style={{ position: "absolute", display: "none" }}>
            {step.content}
          </pre>
        )}
      </button>
      ...
      >
        {messages.length === 0 && (
          ...
          >
            ...
            >
              Give me a task. I can search the web, read URLs, write files, and
              reason across multiple steps to get it done.
            </div>
            <HomeAmbient />
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: "8px", alignItems: "flex-end" }}>
          ...
          <button
            ...
            style={{
              ...
              background: loading
                ? "var(--surf3)"
                : "linear-gradient(135deg, #c4485a, #d4956a)",
              color: "#fff",
              fontSize: "16px",
              cursor: loading ? "not-allowed" : "pointer",
              flexShrink: 0,
