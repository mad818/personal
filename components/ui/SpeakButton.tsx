...
// ── SpeakButton ───────────────────────────────────────────────────────────────
export function SpeakButton({ text, maxChars = 1200, size = "sm" }: SpeakButtonProps) {
  ...
  const voiceSettings = useStore((s) => ({
    speakRate:  (s.settings as Record<string, unknown>).speakRate  as number | undefined,
    speakPitch: (s.settings as Record<string, unknown>).speakPitch as number | undefined,
    speakVoice: (s.settings as Record<string, unknown>).speakVoice as string | undefined,
  }))
  ...
  return (
    ...
    >
      {speaking ? (
        // Stop icon
        <svg width={iconSize} height={iconSize} viewBox="0 0 12 12" fill="currentColor">
          <rect x="2" y="2" width="8" height="8" rx="1" />
        </svg>
      ) : (
        // Speaker icon
        <svg width={iconSize} height={iconSize} viewBox="0 0 12 12" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round">
          <path d="M2 4.5h2l3-3v9l-3-3H2z" fill="currentColor" stroke="none" opacity={0.9} />
