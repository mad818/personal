...
const RADAR_STAGE_GUIDANCE: Record<VehicleRadarProcessingStage, string> = {
  capture:
    "Capture keeps the note on raw passive ingest assumptions, bench source labels, and what was actually observed.",
  preprocess:
...
function stringifyVehicleFlightSessionBundle(
...
export default function VehicleArtifactManifestCard() {
  ...
  const benchChecklistState = useStore(
  ...
  const connectorProfileRaw = useStore(
  ...
  const [importedBundle, setImportedBundle] =
    useState<VehicleFlightSessionBundle | null>(null);
  const [importError, setImportError] = useState<string | null>(null);
  const [importVaultStatus, setImportVaultStatus] =
    useState<VaultStatus>("idle");
  const [renderTarget, setRenderTarget] =
    useState<VehicleRenderBriefTarget>("camera_mount");
  const [renderGoal, setRenderGoal] = useState("");
  const [renderVaultStatus, setRenderVaultStatus] =
    useState<VaultStatus>("idle");
  ...
  const futurePreviewFiles = useMemo(
    () =>
      bundle.manifest.files.filter(
        (file) =>
          file.kind === "preview_3d" || file.kind === "parametric_source",
      ),
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
      ...
      >
        <div
          style={{ fontSize: "11px", fontWeight: 700, color: "var(--text2)" }}
        >
        ...
        >
          Export stays local to the browser. Import reads a previously exported
          JSON bundle and lets you file a clean session summary into Vault
          without requiring the aircraft to exist yet.
        </div>
        {bundle.radar ? (
          <div
            style={{
          ...
          >
            Radar readiness attached: {bundle.radar.modeLabel} ·{" "}
            {
              VEHICLE_RADAR_PROCESSING_STAGE_LABELS[
                bundle.radar.processingStage
              ]
            }
            .
          </div>
      ...
      >
        ...
        >
          {replayScenario.summary} Incident type:{" "}
          {replayScenario.vaultPackage.incidentType.replace(/_/g, " ")}.
        </div>
        <div
          className="nexus-shell-inline-list"
          aria-label="Replay package tags"
          style={{ marginTop: "8px" }}
        >
      ...
      >
        {[
          ...
          { label: "Source", value: bundle.manifest.sourceMode },
          {
            ...
            value:
              VEHICLE_CONNECTOR_TRANSPORT_LABELS[
                bundle.connectorProfile.transport
              ],
          },
        ].map((item) => (
          <div
      ...
      >
        ...
        <div
          style={{ fontSize: "10px", color: "var(--text3)", lineHeight: 1.6 }}
        >
          Future drone sessions can carry a local preview model and its
          parametric source beside the normal telemetry bundle. That gives
          mounts, brackets, enclosure ideas, or fit checks a clear home when
          hardware and model files start to exist.
        </div>
        <div
          style={{
            display: "grid",
        ...
        >
          ...
          >
            {Object.entries(VEHICLE_RENDER_BRIEF_TARGET_LABELS).map(
              ([value, label]) => (
        ...
        <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
          <button
            type="button"
            onClick={() =>
              void copyTextWithFeedback(renderBrief, "Render brief")
            }
            className="nexus-shell-button"
          >
            Copy render brief
        ...
        {renderVaultStatus === "saved" ? (
          <div style={CONTINUATION_CARD_STYLE}>
            <div
              style={{ fontSize: "10px", fontWeight: 700, color: "#a7f3d0" }}
            >
      ...
      >
        ...
        <div
          style={{ fontSize: "10px", color: "var(--text3)", lineHeight: 1.6 }}
        >
          Use this optional block to carry passive radar and future
          sensor-fusion notes inside the same session bundle. It describes
          readiness only and never represents RF control or flight authority.
        </div>
        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
          {Object.entries(VEHICLE_RADAR_PROCESSING_STAGE_LABELS).map(
            ([value, label]) => (
              <span
                key={value}
                style={{
        ...
        >
          <select
            ...
            value={radarProcessingStage}
            onChange={(event) =>
              setRadarProcessingStage(
                event.target.value as VehicleRadarProcessingStage,
              )
            }
            style={{
              minHeight: "38px",
        ...
        {importedBundle ? (
          ...
          >
            ...
            {importedBundle.radar ? (
              <div
                style={{ fontSize: "10px", color: "#bfdbfe", lineHeight: 1.55 }}
              >
                Radar readiness: {importedBundle.radar.modeLabel} ·{" "}
                {
                  VEHICLE_RADAR_PROCESSING_STAGE_LABELS[
                    importedBundle.radar.processingStage
                  ]
                }
            ...
            {importedBundle.radar?.artifactLabels.length ? (
              ...
              >
                Artifact labels:{" "}
                {importedBundle.radar.artifactLabels.join(", ")} · Fusion note:{" "}
                {importedBundle.radar.fusionNote}
              </div>
            ) : importedBundle.radar ? (
              <div
                style={{
          ...
          <div
            style={{ fontSize: "11px", color: "var(--text3)", lineHeight: 1.6 }}
          >
            Use this after the first real bench or field session: export the
            bundle here, keep the JSON with your logs, and import it later when
            you want to summarize or file the session into Vault.
          </div>
        )}
      </div>
      ...
      latestRadarSessionSummary ? (
        ...
        >
          ...
          >
            The latest vehicle session, render brief, and radar-attached summary
            stay one click away in VAULT so later bench work reopens the same
            continuity instead of starting fresh.
          </div>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {latestVehicleSessionSummary ? (
              <button
                ...
                onClick={() =>
                  router.push(
                    buildCompiledPageHref(latestVehicleSessionSummary),
                  )
                }
                className="nexus-shell-button"
                style={{
            ...
            {latestRenderBrief ? (
              <button
                type="button"
                onClick={() =>
                  router.push(buildCompiledPageHref(latestRenderBrief))
                }
                className="nexus-shell-button"
                style={{
                  minHeight: "32px",
            ...
            {latestRadarSessionSummary ? (
              <button
                type="button"
                onClick={() =>
                  router.push(buildCompiledPageHref(latestRadarSessionSummary))
                }
                className="nexus-shell-button"
                style={{
                  minHeight: "32px",
