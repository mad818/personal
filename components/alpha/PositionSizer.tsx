        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {[
              {
                label: "Risk $",
                val: `$${calc.riskAmt.toLocaleString("en-US", { maximumFractionDigits: 2 })}`,
                col,
              },
              {
                label: "Stop Dist",
                val: `${calc.slPct.toFixed(2)}%`,
                col: calc.slPct > 5 ? "#ef4444" : "var(--text)",
              },
              {
                label: "Units",
                val:
                  calc.posSize >= 1
                    ? calc.posSize.toFixed(4)
                    : calc.posSize.toFixed(8),
                col: "var(--text)",
              },
              {
                label: "Position $",
                val: fmtPrice(calc.posValue),
                col: "var(--text)",
              },
            ].map((m) => (
              <div
          ...
          {calc.rrRatio !== null && (
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              {[
                {
                  label: "Risk : Reward",
                  val: `1 : ${calc.rrRatio!.toFixed(2)}`,
                  col: calc.rrRatio! >= 2 ? "#10b981" : "#f59e0b",
                },
                {
                  label: "Win if TP",
                  val: `+$${calc.pnlWin!.toLocaleString("en-US", { maximumFractionDigits: 2 })}`,
                  col: "#10b981",
                },
                {
                  label: "Loss if SL",
                  val: `-$${calc.pnlLoss.toLocaleString("en-US", { maximumFractionDigits: 2 })}`,
                  col: "#ef4444",
                },
                {
                  label: "Kelly Pos $",
                  val:
                    calc.kellyPos !== null && calc.kellyPos > 0
                      ? fmtPrice(calc.kellyPos!)
                      : "n/a",
                  col: "var(--accent)",
                },
              ].map((m) => (
