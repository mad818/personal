...
async function checkRouteSmoke(req: NextRequest): Promise<AdapterResult> {
  ...
  const protectedUrls = ['/api/status', '/api/project?section=tree', '/api/tools', '/api/ai']

  const publicChecks = await Promise.all(publicUrls.map(async (u) => {
    try {
      const r = await fetch(`${base}${u}`, { signal: AbortSignal.timeout(6000) })
  ...
  const protectedChecks = await Promise.all(protectedUrls.map(async (u) => {
    try {
      const r = await fetch(`${base}${u}`, { signal: AbortSignal.timeout(6000) })
...
export async function POST(req: NextRequest) {
  try {
    ...
    const adapters = (Array.isArray(body.adapters) ? body.adapters : ['typecheck', 'lint', 'route_smoke']) as VerificationAdapter[]
    const uniq = Array.from(new Set(adapters)).filter((a): a is VerificationAdapter =>
    ...
    for (const adapter of uniq) {
      if (adapter === 'typecheck') {
        ...
        results.push({
          adapter,
          passed: r.ok,
          summary: r.ok ? 'Type check passed' : `Type check failed: ${r.output || 'unknown error'}`,
        })
        continue
      }
      if (adapter === 'lint') {
        const r = await runCommand('npm', ['run', 'lint', '--', '--max-warnings=0'], 180000)
        results.push({
          adapter,
          passed: r.ok,
          summary: r.ok ? 'Lint passed' : `Lint failed: ${r.output || 'unknown error'}`,
        })
        continue
      }
    ...
    } satisfies VerificationResponse, { status: 500 })
  }
}
