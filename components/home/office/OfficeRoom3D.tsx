...
function CinematicCameraRig({
  ...
  useFrame((state, delta) => {
    ...
    const t = state.clock.getElapsedTime();
    positionTarget.set(
      preset.position[0] + Math.sin(t * 0.14) * 0.04 * drift,
      preset.position[1] + Math.cos(t * 0.18) * 0.025 * drift,
      preset.position[2] + Math.sin(t * 0.11 + 0.8) * 0.05 * drift,
    );
    lookAtTarget.set(
      preset.lookAt[0] + Math.sin(t * 0.12) * 0.035 * drift,
      preset.lookAt[1] + Math.cos(t * 0.1) * 0.015 * drift,
      preset.lookAt[2] + Math.cos(t * 0.13) * 0.03 * drift,
    );
    camera.position.lerp(positionTarget, Math.min(1, delta * (1.25 + cue.shadowContrast)));
