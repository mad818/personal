...
export function getSkillBarGradient(level: number): string {
  if (level >= 90)
    return "linear-gradient(to right, var(--burgundy), var(--rose), var(--gold))";
  if (level >= 75)
    return "linear-gradient(to right, var(--burgundy), var(--accent))";
  if (level >= 50)
    return "linear-gradient(to right, var(--burgundy), var(--blush))";
  if (level >= 25)
    return "linear-gradient(to right, var(--surf3), var(--text2))";
...
export function simulateLearning(skill: Skill): LearningEvent {
  ...
  const types: LearningEvent["type"][] = [
    "improved",
    "error_learned",
    "pattern_detected",
    "optimization",
...
export function predictNextMilestone(skill: Skill): {
  level: number;
  ...
  const timeStr =
    days < 1 ? `${hours}h` : days < 7 ? `${days}d` : `${Math.ceil(days / 7)}w`;
...
export const DEFAULT_SKILLS: Skill[] = [
  // INTELLIGENCE & OSINT
  {
    ...
    version: "2.3",
    abstract:
      "Processes 50+ news sources with bias detection and entity extraction.",
    overview:
      "Triggered when the agent needs to analyze current events, media narratives, or breaking news. Aggregates Guardian, Reuters, and RSS feeds. Applies bias classifiers and entity extraction. Prerequisites: guardian API key (optional). Returns structured summaries with bias scores and named entities.",
    triggerPhrases: [
      "analyze news",
      "what is in the news",
      "latest headlines",
  ...
  },
  {
    ...
    learned: D(150),
    improvements: ["Market correlation layer", "Alert threshold tuning"],
    successRate: 0.84,
    totalRuns: 780,
    failureCount: 125,
    version: "1.2",
    abstract:
      "Correlates geopolitical events with market and security impact scores.",
    overview:
      "Triggered when monitoring geopolitical developments, security incidents, or market-moving events. Ingests news analysis and cross-references with historical event impact data. Outputs threat level (1-5) with affected domains. Prerequisites: news-analysis skill. Best used after news-analysis runs.",
    triggerPhrases: [
      "assess threat",
      "geopolitical risk",
      "security risk",
  ...
  },
  {
    ...
    learned: D(120),
    improvements: ["Time-series FFT added"],
    successRate: 0.79,
    totalRuns: 620,
    failureCount: 130,
    version: "1.1",
    abstract:
      "Identifies recurring temporal and statistical patterns across data streams.",
    overview:
      "Triggered on time-series data analysis requests or anomaly detection tasks. Applies FFT decomposition, sliding window correlation, and frequency analysis. Works on price data, sensor readings, and log streams. Prerequisites: raw numeric or text data stream. Returns detected patterns with confidence scores.",
    triggerPhrases: [
      "find patterns",
      "anomaly detection",
      "recurring events",
  ...
  },
  {
    ...
    learned: D(90),
    improvements: ["Domain reputation DB integrated"],
    successRate: 0.77,
    totalRuns: 380,
    failureCount: 87,
    version: "1.1",
    abstract:
      "Cross-references claims against multiple independent sources for credibility.",
    overview:
      "Triggered when fact-checking claims, verifying breaking news, or assessing source credibility. Checks domain reputation, publication history, and cross-references against known authoritative sources. Prerequisites: news article URL or claim text. Returns credibility score with supporting evidence.",
    triggerPhrases: [
      "verify source",
      "fact check",
      "is this true",
  ...
  },
  {
    ...
    learned: D(130),
    improvements: ["IOC enrichment pipeline"],
    successRate: 0.81,
    totalRuns: 560,
    failureCount: 106,
    version: "1.2",
    abstract:
      "Proactively hunts threats by enriching IOCs and correlating network indicators.",
    overview:
      "Triggered for proactive security monitoring, IOC investigation, or when anomalous network activity is detected. Enriches IP/domain/hash indicators via OTX and threat feeds. Correlates against known attack patterns. Prerequisites: OTX API key (optional). Returns IOC report with maliciousness probability and MITRE ATT&CK mappings.",
    triggerPhrases: [
      "threat hunt",
      "ioc lookup",
      "check ip reputation",
  ...
  },
  {
    ...
    learned: D(60),
    improvements: ["Playbook v1.2"],
    successRate: 0.72,
    totalRuns: 240,
    failureCount: 67,
    version: "1.2",
    abstract:
      "Executes automated triage playbooks for detected security incidents.",
    overview:
      "Triggered when a security alert is elevated to incident status. Runs initial triage checklist: asset identification, containment options, evidence collection guidance. Generates incident report template. Prerequisites: security alert with type and severity. Works best after threat-hunting or cve-analysis provides context.",
    triggerPhrases: [
      "incident response",
      "security incident",
      "breach detected",
  ...
  // PHYSICAL SECURITY
  {
    ...
    learned: D(45),
    improvements: ["Motion sensitivity tuning"],
    successRate: 0.88,
    totalRuns: 180,
    failureCount: 22,
    version: "1.1",
    abstract:
      "Processes RTSP camera streams with motion detection and clip recording.",
    overview:
      "Triggered for camera feed monitoring, motion alert configuration, or live view requests. Connects to RTSP streams (H.264/H.265), applies motion detection with configurable sensitivity zones. Records 30s clips on trigger. Prerequisites: RTSP URL with credentials. Best sensitivity setting: 3 consecutive frames for false-positive suppression.",
    triggerPhrases: [
      "camera feed",
      "motion detected",
      "monitor camera",
  ...
  },
  {
    ...
    version: "1.0",
    abstract:
      "Monitors defined perimeter zones and triggers graduated alert responses.",
    overview:
      "Triggered for perimeter breach events or zone configuration requests. Implements two-zone alerting: outer zone (5-20m) triggers silent alert + camera focus; inner zone (0-5m) triggers immediate alert + lights + siren. Prerequisites: camera-monitoring + object-detection active. Logs all events with RTSP snapshots.",
    triggerPhrases: [
      "perimeter breach",
      "zone alert",
      "intruder detected",
  ...
  },
  {
    ...
    version: "1.0",
    abstract:
      "Executes collision avoidance and safe-stop procedures on emergency triggers.",
    overview:
  ...
  },
  {
    ...
    version: "1.0",
    abstract:
      "Monitors all system components and predicts maintenance needs from sensor trends.",
    overview:
      "Triggered for system health checks, pre-mission readiness assessments, or maintenance scheduling. Monitors RTX 5070 VRAM/thermal, motor current draw, battery state of charge, and GPS lock quality. Raises warnings at configurable thresholds. Prerequisites: access to system metrics endpoints. Returns health dashboard JSON with status per component.",
    triggerPhrases: [
      "system health",
      "diagnostics check",
      "self diagnostic",
  ...
  // IoT & AUTOMATION
  {
    ...
    learned: D(80),
    improvements: ["Auto-discovery added", "MQTT v5 support"],
    successRate: 0.87,
    totalRuns: 310,
    failureCount: 40,
    version: "1.2",
  ...
  },
  {
    ...
    learned: D(70),
    improvements: ["Conditional branching"],
    successRate: 0.83,
    totalRuns: 240,
    failureCount: 41,
    version: "1.1",
    abstract:
      "Executes event-driven automation rules with conditional branching.",
    overview:
      "Triggered when configuring home automation, creating new rules, or responding to device events. Implements IFTTT-style logic with AND/OR conditions, delays, and schedules. Supports device-management events as triggers. Prerequisites: device-management active. Rules are persisted in Zustand store with enable/disable toggles.",
    triggerPhrases: [
      "create automation",
      "if this then that",
      "automation rule",
  ...
  },
  {
    ...
    version: "1.0",
    abstract:
      "Predicts equipment failures from sensor trend patterns before they occur.",
    overview:
      "Triggered for predictive maintenance scheduling, anomaly alerts from device-management, or manual health checks. Analyzes vibration, temperature, and runtime trends to predict failure windows. Uses rolling Z-score for anomaly detection. Prerequisites: device-management with 7+ days of sensor history. Returns failure probability with recommended maintenance window.",
    triggerPhrases: [
      "predictive maintenance",
      "failure prediction",
      "equipment health",
  ...
  },
  {
    ...
    learned: D(85),
    improvements: ["Fear & Greed integration"],
    successRate: 0.8,
    totalRuns: 320,
    failureCount: 64,
    version: "1.2",
    abstract:
      "Aggregates market sentiment from news, Fear & Greed index, and social signals.",
    overview:
      "Triggered for market mood analysis, contrarian signal detection, or sentiment dashboards. Integrates Fear & Greed index (Alternative.me), news sentiment scoring from news-analysis, and optional social feed analysis. Validated at 0.71 Pearson correlation with price action. Prerequisites: news-analysis active. Returns composite sentiment score (-100 to +100) with trend direction.",
    triggerPhrases: [
      "market sentiment",
      "fear and greed",
      "investor mood",
...
export const DEFAULT_LEARNING_LOG: LearningEvent[] = [
  {
    id: "log-001",
    skillId: "cve-analysis",
    skillName: "CVE Analysis",
    type: "optimization",
  ...
  {
    id: "log-002",
    skillId: "price-analysis",
    skillName: "Price Analysis",
    type: "pattern_detected",
    description:
      "BTC/ETH correlation above 0.94 — highest in 90 days, flagging as signal",
    timestamp: T(1.2),
    xpGained: 60,
  },
  {
    id: "log-003",
    skillId: "news-analysis",
    skillName: "News Analysis",
    type: "improved",
    description:
      "Bias detection model retrained on 2,400 new labeled samples — F1 +0.07",
    timestamp: T(2.5),
    xpGained: 90,
  },
  {
    id: "log-004",
    skillId: "threat-hunting",
    skillName: "Threat Hunting",
    type: "error_learned",
    description:
      "False positive rate on IP reputation checks was 18% — threshold recalibrated",
    timestamp: T(5),
    xpGained: 35,
  },
  {
    id: "log-005",
    skillId: "momentum-detection",
    skillName: "Momentum Detection",
    type: "pattern_detected",
    description:
      "Identified double-bottom formation across 4 assets simultaneously",
    timestamp: T(8),
    xpGained: 55,
  },
  {
    id: "log-006",
    skillId: "camera-monitoring",
    skillName: "Camera Monitoring",
    type: "improved",
    description:
      "Motion sensitivity tuned — false triggers from lighting changes reduced 60%",
    timestamp: T(12),
    xpGained: 40,
  },
  {
    id: "log-007",
    skillId: "device-management",
    skillName: "Device Management",
    type: "optimization",
    description:
      "MQTT broker reconnect backoff improved — zero missed messages in last 6h",
    timestamp: T(18),
    xpGained: 30,
  },
  {
    id: "log-008",
    skillId: "self-diagnostics",
    skillName: "Self-Diagnostics",
    type: "acquired",
    description:
      "RTX 5070 VRAM monitoring module integrated — thermal throttle detection active",
    timestamp: T(24),
    xpGained: 120,
  },
  {
    id: "log-009",
    skillId: "source-verification",
    skillName: "Source Verification",
    type: "error_learned",
    description:
      "Reuters feed returned stale data for 4h — staleness detection added",
    timestamp: T(36),
    xpGained: 50,
  },
  {
    ...
    type: "improved",
    description:
      "Fear & Greed index correlation with price action validated at 0.71 Pearson",
    timestamp: T(48),
    xpGained: 70,
  },
];
...
export const DEFAULT_KNOWLEDGE: KnowledgeEntry[] = [
  {
    ...
    content:
      "Standard RTSP stream configuration for IP cameras. Use rtsp://user:pass@ip:554/stream1 format. Enable H.264 encoding at 15fps for optimal CPU/bandwidth balance. Set keyframe interval to 2s for motion detection accuracy. Recommended: Reolink, Hikvision with ONVIF profile S compliance.",
  ...
  },
  {
    ...
    content:
      "Hierarchical topic design: home/{room}/{device}/{property}. Use retained messages for last known state. QoS 1 for sensor data, QoS 2 for critical actuator commands. Prefix with version: v2/home/... for schema migrations. Mosquitto broker with TLS on port 8883. Home Assistant auto-discovery via homeassistant/{component}/{node_id}/config.",
  ...
  },
  {
    ...
    content:
      "Entry: RSI(14) crosses above 55 with volume >1.5x 20-day avg AND linear regression slope positive over 7 candles. Exit: RSI crosses below 45 OR trailing stop 8% below entry. Best timeframe: 4H crypto, 1D equities. Avoid entries during high-impact macro events (FOMC, CPI). Backtested 2021-2025: 58% win rate, 1.9 reward/risk ratio.",
  ...
  },
  {
    ...
    category: "Vehicle Operations",
    content:
      "FLIR Lepton 3.5 calibration procedure for DonkeyCar night ops. NUC (non-uniformity correction) at startup, 30s warm-up required. Map thermal range 20-40°C for human detection. Fuse with RGB camera using homographic transform — calibrate with checkerboard target at 1m, 2m, 4m distances. Store transformation matrix in config. Expected accuracy: ±15cm at 4m range.",
  ...
  },
  {
    ...
    category: "Security Protocols",
    content:
      "Zone-based perimeter alerting: Inner zone (0-5m) = immediate alert + lights + siren. Outer zone (5-20m) = silent alert + camera focus. Motion above threshold triggers 30s recording clip. Classifier: person > vehicle > animal priority. Night mode activates IR floodlights on outer zone trigger. All events logged to SQLite with RTSP snapshot. False positive suppression: 3 consecutive frames required.",
  ...
  },
  {
    ...
    category: "IoT Configurations",
    content:
      "ESP32-S3 with ESPHome for sensor nodes. Power: 18650 LiPo + solar panel for outdoor nodes, target 95% uptime. Deep sleep between readings: 30s cycle = ~6 month battery life. Sensors: BME280 (temp/humidity/pressure), DS18B20 (waterproof temp), PIR HC-SR501. OTA updates via ESPHome API. Watchdog timer: 60s. Partition scheme: minimal SPIFFS for larger firmware.",
  ...
  },
  {
    ...
    category: "System Architecture",
    content:
      "Rate limit matrix: NVD API free tier 5 req/30s (use API key for 50/30s). CoinGecko free 10-30 req/min (Demo key 30/min). Guardian API 1 req/s. OTX 10k req/day. FRED unlimited. Implement exponential backoff: 1s, 2s, 4s, 8s max. Cache responses: prices 30s, CVEs 15min, news 5min. Queue system for burst requests. Monitor via /api/status endpoint.",
