...
function calcEntropy(): number {
  ...
  try {
    ...
    bits += Math.log2(((navigator.languages as string[] | undefined) ?? [navigator.language ?? '']).length + 1) + 2
    bits += 2
    ...
    bits += Math.log2(((navigator as unknown as {deviceMemory?: number}).deviceMemory ?? 1) + 1)
...
function probeWebRTC(): Promise<string | null> {
  return new Promise(resolve => {
    try {
      ...
      let resolved = false
      const cleanup = () => { try { pc.close() } catch (_) {} }
      pc.createDataChannel('')
      pc.createOffer().then(o => pc.setLocalDescription(o)).catch(() => { cleanup(); resolve(null) })
      pc.onicecandidate = e => {
        if (!e?.candidate) {
          if (!resolved) { resolved = true; cleanup(); resolve(found.size > 0 ? Array.from(found)[0] : null) }
          return
        }
...
function DotIcon({ result }: { result: Check['result'] }) {
  const col =
    result === 'ok'       ? '#10b981'
    : result === 'warn'   ? '#ef4444'
    : result === 'checking' ? '#f59e0b'
    : 'var(--text3)'
  return (
    <span style={{
      display: 'inline-block', width: '8px', height: '8px', borderRadius: '50%',
      background: col, boxShadow: result !== 'idle' ? `0 0 6px ${col}` : 'none',
...
export default function OpsecPanel() {
  ...
  async function runCheck() {
    ...
    updateCheck('https', {
      ...
      note:   isHttps ? 'Served over HTTPS' : 'Not on HTTPS — keys exposed to network',
    })
    if (isHttps) s += 30
    ...
    let isTor = false
    try {
      const tr = await fetch('https://check.torproject.org/api/ip', { signal: AbortSignal.timeout(4000) })
      const td = await tr.json()
      isTor = !!td.IsTor
    } catch (_) {}
    updateCheck('tor', {
      result: 'ok',
      note:   isTor ? 'Tor exit node detected — high anonymity' : 'Not routed through Tor',
    })
    s += 20
  ...
  const label = score === null ? '—'
    : score >= 90 ? 'Strong OPSEC'
    : score >= 60 ? 'Moderate OPSEC'
    : score >= 30 ? 'Weak OPSEC'
    : 'Poor OPSEC'

  const scoreCol = score === null ? 'var(--text3)'
    : score >= 90 ? '#10b981'
    : score >= 60 ? '#f59e0b'
    : '#ef4444'
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
        <div>
          <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text)', fontFamily: 'monospace' }}>
            OPSEC Score: <span style={{ color: scoreCol, fontSize: '18px' }}>{score ?? '—'}</span>
            {score !== null && <span style={{ fontSize: '11px', color: 'var(--text3)', marginLeft: '6px' }}>{label}</span>}
          </div>
          <div style={{ fontSize: '10px', color: 'var(--text3)', marginTop: '2px' }}>All checks run locally — nothing leaves your browser</div>
        </div>
        <button
          ...
          style={{
            ...
            background: running ? 'var(--surf3)' : 'var(--accent)', color: '#fff',
            fontWeight: 700, fontSize: '12px', cursor: running ? 'not-allowed' : 'pointer',
          }}
        >
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {checks.map(c => (
          <div key={c.id} style={{
            ...
            <div>
              <div style={{ fontSize: '12px', fontWeight: 700, color: 'var(--text)' }}>{c.label}</div>
              {c.note && <div style={{ fontSize: '11px', color: 'var(--text3)', marginTop: '2px' }}>{c.note}</div>}
            </div>
          </div>
        ))}
      </div>
