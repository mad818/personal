...
import { getAutoJobsForMode, isAutoOpsModeEnabled, type AutoModeJob } from '@/lib/autoOpsJobs'
...
export default function CronSchedulerRunner() {
  ...
  const runAutoJob = useCallback(async (autoJob: AutoModeJob, force = false) => {
    ...
    try {
      ...
      autoStatus = 'error'
      autoSummary = e instanceof Error ? e.message.slice(0, 200) : 'Mode auto job failed.'
    }
    ...
    const relatedTab = mode === 'war' ? 'cyber' : mode === 'nightOps' ? 'security' : 'intel'
    addModeBriefing({
      mode,
      jobId: autoJob.id,
    ...
    if (mode === 'nightOps' && autoStatus === 'ok') {
      ...
      try {
        await apiFetch('/api/tools', {
          ...
          body: JSON.stringify({
            tool: 'write_file',
            input: { filename: 'night-ops-handoff-latest.md', content: handoff },
          }),
        })
      } catch {
  ...
  useEffect(() => {
      ...
      try {
        ...
        for (const job of due) {
          ...
          try {
            ...
            status = 'error'
            summary = e instanceof Error ? e.message.slice(0, 200) : 'Scheduled task failed.'
          }

          const runAt = Date.now()
          nextJobs = nextJobs.map((j) =>
            j.id === job.id ? { ...j, lastRunAt: runAt, lastStatus: status, lastSummary: summary } : j,
          )
    ...
    void tick()
    window.addEventListener('nexus-auto-ops-run-now', onManualRun as EventListener)
