  ...
  return (
    ...
    >
      <div
        style={{
          fontWeight: 700,
          color: magColor(d.mag),
          marginBottom: "4px",
          fontFamily: "monospace",
        }}
      >
        M{d.mag.toFixed(1)} —{" "}
        {magColor(d.mag) === CHART.red
          ? "MAJOR"
          : d.mag >= 6
            ? "STRONG"
            : d.mag >= 5
      ...
      <div style={{ fontSize: "11px", color: CHART.text, marginBottom: "3px" }}>
        {d.place}
      </div>
  ...
  const data = useMemo(() => {
    return earthquakes
      .map((eq: any) => {
        ...
        const place =
          eq.place ?? eq.location ?? eq.properties?.place ?? "Unknown";
    ...
    >
      ...
      <ResponsiveContainer width="100%" height={180}>
        <ScatterChart margin={{ left: 0, right: 10, top: 4, bottom: 4 }}>
          ...
          />
          <Scatter data={data} shape={<MagDot />} />
        </ScatterChart>
      </ResponsiveContainer>
      ...
      <div style={{ display: "flex", gap: "14px", marginTop: "8px" }}>
        {[
          ...
          >
            ...
            />
            <span
              style={{ fontSize: "9px", color: CHART.text3, fontWeight: 600 }}
            >
              {label}
            </span>
          </div>
