...
function cvesToTriageItems(cves: CVE[]): TriageItem[] {
  return cves
    ...
    .map((c) => ({
      ...
      priority: (c.severity === 'CRITICAL' ? 'critical' : 'high') as TriagePriority,
      title:    c.id,
      detail:   c.description || 'No description.',
      score:    c.score ? (c.score / 10) * 100 : (c.severity === 'CRITICAL' ? 90 : 70),
...
function otxToTriageItems(pulses: OTXPulse[]): TriageItem[] {
  return pulses
    ...
    .map((p) => ({
      ...
      title:    p.name,
      detail:   p.description || (p.tags.length ? `Tags: ${p.tags.slice(0, 4).join(', ')}` : 'No description.'),
...
function kevToTriageItems(kevs: KEVEntry[]): TriageItem[] {
  return kevs.map((k) => {
    ...
    return {
      ...
      priority: (overdue ? 'critical' : urgent ? 'high' : 'medium') as TriagePriority,
      title:    `${k.cveID} — ${k.product}`,
      detail:   k.shortDescription || k.vulnerabilityName || '',
      score:    overdue ? 95 : urgent ? 80 : 60,
      badge:    days !== null ? (days < 0 ? `${Math.abs(days)}d OVERDUE` : days === 0 ? 'Due today' : `Due in ${days}d`) : '',
  ...
  return (
    <a
      ...
      style={{
        ...
        background: 'var(--surf2)', borderRadius: '10px',
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '6px', flexWrap: 'wrap' }}>
        <span style={{
          ...
          background: `${sCol}22`, color: sCol, letterSpacing: '.5px',
        }}>
          {item.source}
        </span>
        <span style={{
          fontSize: '9.5px', fontWeight: 800, padding: '1px 7px', borderRadius: '6px',
          background: `${pCol}18`, color: pCol, textTransform: 'uppercase', letterSpacing: '.4px',
        }}>
          {item.priority}
        </span>
        {item.badge && (
          <span style={{
            ...
            padding: '1px 7px', borderRadius: '6px', background: 'var(--surf3)',
          }}>
            {item.badge}
          </span>
        )}
        {item.age && (
          <span style={{ fontSize: '9.5px', color: 'var(--text3)', marginLeft: 'auto' }}>
            {item.age}
          </span>
        )}
      </div>
      ...
      {/* Title */}
      <div style={{
        fontSize: '12.5px', fontWeight: 700, color: 'var(--text)',
        marginBottom: '4px', fontFamily: item.source === 'CVE' ? 'monospace' : 'inherit',
      }}>
        {item.title}
      </div>
      ...
      {/* Detail */}
      <div style={{
        fontSize: '11px', color: 'var(--text2)', lineHeight: 1.45,
    ...
    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '16px' }}>
      {[
        ...
        <div key={label} style={{
          ...
          <div style={{ fontSize: '9px', fontWeight: 700, color: 'var(--text3)', letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: '2px' }}>{label}</div>
...
export default function TriageView() {
  ...
  const hasData = cves.length > 0 || otxPulses.length > 0 || kevEntries.length > 0
  ...
  if (!hasData) {
    return (
      <div style={{ padding: '60px', textAlign: 'center', color: 'var(--text3)', fontSize: '13px' }}>
        ...
        <div style={{ fontSize: '11px', marginTop: '6px' }}>CVEs and OTX pulses load automatically. CISA KEV is fetched here.</div>
      </div>
    )
  }
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {(['all', 'critical', 'high'] as PriorityFilter[]).map((f) => (
            <button key={f} onClick={() => setPriorityFilter(f)} style={{
              height: '26px', padding: '0 10px', borderRadius: '6px', fontSize: '10px', fontWeight: 700,
              border: '1px solid var(--border2)', cursor: 'pointer', textTransform: 'uppercase',
              background: priorityFilter === f ? PRIORITY_COLOR[f as TriagePriority] ?? 'var(--accent)' : 'transparent',
        ...
        <div style={{ display: 'flex', gap: '4px' }}>
          {(['all', 'CVE', 'OTX', 'CISA'] as SourceFilter[]).map((f) => (
            <button key={f} onClick={() => setSourceFilter(f)} style={{
              ...
              background: sourceFilter === f ? (SOURCE_COLOR[f as TriageSource] ?? 'var(--accent)') : 'transparent',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
        {visible.length === 0 && (
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text3)', fontSize: '13px' }}>
            No items match this filter. Try widening the priority or source filter.
          </div>
        )}
        {visible.map((item) => <TriageCard key={item.id} item={item} />)}
      </div>
    </div>
  )
}
