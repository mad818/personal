...
90. Application route-policy isolation is not the same as packaged-desktop or OS-level no-outbound proof. Evaluate the complete policy inventory rather than a few sample routes, verify live posture only through safe status endpoints, and keep packaged-shell provenance plus packet-level evidence as explicit blockers even when every application-level connector and high-risk policy is correctly denied.
91. Root instructions and generated handoffs must resolve against current repository facts. Derive framework majors from `package.json`, compare GA routes with `lib/release-matrix.json`, verify every named skill/page/guide exists, remove dead instruction paths, and label legacy compatibility guidance as non-canonical so a fresh session cannot start from plausible but obsolete prose.
92. Publication scanners reasonably treat secret-like assignment names as sensitive even when a value is synthetic. Name deterministic static credentials as auth fixtures rather than token/secret assignments, preserve the exact fixture behavior, and rerun both the feature fixture and publication gate instead of weakening the scanner or adding an exception.
93. Documentation truth needs an explicit current-versus-historical boundary. Gate public README text, visible graphics, architecture, design rules, operator guidance, manifests, and release-matrix routes as current; preserve dated plan bodies under prominent historical notices and remove them from current authority chains instead of either silently rewriting history or letting snapshot claims masquerade as live state.
94. Distinct signed authentication tiers must be enforced at the gateway with method-plus-exact-path policy, not collapsed into one authenticated boolean. Keep read methods available, make mutation exceptions explicit and runtime-tested, force restricted tiers onto their promised provider boundary, and default-deny future mutation routes before their handlers run.
95. Framework lint presets should own their matching parser/plugin pair unless the project has a proven custom parsing requirement. Before pinning a root parser, inspect `eslint --print-config` and the lock graph; when removing a stale override, compare the complete rule/settings map and gate the effective parser's TypeScript peer range so warning cleanup cannot silently reduce coverage.
96. A static CSP cannot safely authorize intentional raw boot scripts without authorizing every injected inline script too. Generate per-request script nonces in middleware, pass the same policy to rendering and the response, attach the nonce to every intentional raw script, retain development-only exceptions explicitly, and gate both the executable policy and the active script inventory.
97. Third-party widget scripts at mutable, unversioned URLs cannot receive a durable SRI claim. When the provider's bootstrap exists only to inject an iframe, execute it in a fixed no-referrer document constrained by both an iframe sandbox and CSP sandbox without `allow-same-origin`, scope its hosts to that route, and keep normal application documents free of the provider's script authority.
98. A successful Next.js compile and page-data collection do not prove the Windows production build when the static-generation worker exits silently at `0/N`. Reproduce the standard build, use `--debug-prerender` only to isolate the optimized worker path, then retain the smallest config change that makes the ordinary `npm run build` pass; in this repo's Next 15 line, disabling server minification fixed the repeated silent worker exit and must remain toolchain-gated.
99. A failed settings read is unknown state, not evidence that server-side keys are unset. Use one `response.ok`-checked loader for initial and post-save reads, render loading/error separately from verified booleans, ignore stale completions, and keep successful writes successful when only the follow-up status refresh fails.
100. A connector outage must not become an empty verified catalog. Proxy routes should return safe non-2xx failures, clients should validate payloads before replacing retained data, and user-facing security or diagnostic panels must keep loading, verified-empty, retained-with-error, and unavailable states distinct with accessible retry.
101. Browser RECON must never contact third-party providers or carry BYOK credentials directly. Route a closed operation allowlist through a protected connector API, validate each target, bound upstream time and response size, keep keys server-side, expose only fixed safe failures, and remove obsolete provider hosts from browser CSP.
102. Market-data panels must not carry provider keys or collapse independent sources into one silent all-or-nothing load. Keep credentials and fixed provider URLs server-side, validate finite quote shapes, return non-2xx when no verified data exists, preserve successful sources and retained client data, and remove stale provider hosts from browser CSP.
103. Public-feed proxy success must mean at least one normalized verified record. Validate and cap client inputs plus upstream payloads on the server, return a fixed safe non-2xx response on total failure, preserve prior client data with visible retry, and never invent a neutral quote or probability when provider evidence is missing.
104. Missing sentiment evidence is unknown, not neutral. Load one normalized snapshot into every supported store shape, remove competing fetch paths, preserve the last verified value on failure, exclude unknown inputs from derived scores, and expose retained/unavailable state with local retry instead of substituting `50`.
105. Company Map source posture is an executable contract, not documentation-only metadata. When a source moves from review-first to adapted, update the static inventory, runtime posture assertions, department membership proof, parity matrix, and operator-facing benefits in the same patch so the focused gate cannot preserve a stale integration state.
106. On this Windows Next.js path, do not repeat lint and TypeScript validation inside a production build immediately after the canonical gate. If the ordinary build compiles but the optimized prerender worker exits silently at `0/N`, use `--debug-prerender` only to rule out a page exception, restore its generated config edits, then run the repository's supported post-verify build path with `NEXUS_NEXT_SKIP_BUILD_CHECKS=1`; never call that green unless canonical lint/type proof already passed and every production page plus performance budget completes.
107. `docs/SYSTEM_STATE.md` cannot be canonical if it is absent from the current-documentation gate or allowed to lag the task queue. Select the first top-level completed identifier in `tasks/todo.md` → `Next Up`, ignore unchecked and nested boxes, and require the first `Latest Shipped` entry to match so a green docs check proves recency instead of only framework/version consistency.
108. A current-context file needs a hard retention budget as well as recency. Keep `SYSTEM_STATE.md` → `Latest Shipped` to a small newest-first window, enforce both entry and character limits, and rely on Git for old shipment history; otherwise a factually correct section can still consume most of every project-context read and bury the operational state it exists to expose.
109. Generated restart guidance must consume the same actionable-queue classifier as the operator command. An independent unchecked-task parser can promote physical, remote-only, or excluded work back into “what's next” even when the authoritative selector correctly reports zero ready work; share the classifier, cap ready titles, and reduce blocked/excluded work to counts.
110. Active component health must be measured from application roots, not by file existence or legacy validators. Follow static, re-exported, and literal lazy imports from `app/`; remove unreachable files with no current evidence, and keep evidence-bearing detached files in an explicit restore-or-retire inventory so green checks cannot silently treat dead UI as shipped behavior.
111. An evidence-bearing detached component still is not shipped behavior. Restore useful capability only through a deliberate current route seam with truthful unavailable/retained posture; otherwise retire the UI, its component-only support code, and stale validators, then point source parity at a reachable replacement or return the capability to `pending` with a zero-exception reachability gate.
112. A reachable panel can still depend on a status field the server never returns, and a library file can still be dead despite a green file-existence validator. Trace the complete component-to-route contract, sanitize configuration posture before returning it, remove uncalled execution exports, and extend app-root reachability to hooks/libraries with only exact package-command-owned script helpers exempted.
113. A current-queue regression fixture must validate classifier invariants, not freeze the task name or actionable count from the tranche that created it. Compare `next` with the first task the classifier itself marks actionable, require matching counts and reason, and keep fixed-name assertions only for durable blocker contracts.
114. A script is maintained only when a current package command, a reachable runtime importer, or a deliberate private exclusion owns it. Source-parity/file-existence references do not prove execution, missing package targets must fail immediately, and background fetch/stage/commit/push automation must not survive without an explicit current operator action and approval boundary.
115. Generated desktop/runtime snapshots must not become a second tracked source tree. Keep the root manifest and current Tauri launcher/config authoritative, ignore generated runtime output, and retire snapshots that duplicate live files, advertise stale commands, or embed local absolute paths; preserve local ignored caches instead of deleting them as part of source cleanup.
116. A security lockfile patch and the currently installed dependency tree are separate evidence. Validate the lockfile with a package-lock-only audit, enforce every affected major through an executable floor check, and run the canonical repository gate; if registry access prevents a fresh install, report stale `node_modules` and missing Cargo cache proof explicitly instead of treating `npm ls` or an offline compile failure as a lockfile regression.
117. An unavailable optional analysis service must not return invented sample findings under HTTP 200, and a displayed confidence value must come from measured evidence rather than coordinates or deterministic decoration. Return explicit unavailable state with an empty result set, label heuristic output by what it actually measures, and keep model-backed upgrades separate from the truthful local fallback.
118. A Windows launcher is not operational merely because it spawned a shell and eventually opened a URL. Validate the exact health payload before opening HQ, bound every wait, distinguish a foreign port from a healthy Nexus runtime, and stop owned Next.js processes through a managed parent-child shutdown handshake with independent port-release proof; do not depend on external process-tree kill authority or leave interrupted smoke runtimes behind.
119. Code-scanning closure must follow the shared taint boundary and a fresh analysis of the exact published ref, not line-by-line alert dismissal. Contain dynamic paths after resolution, pin or validate request origins with redirects disabled, remove raw HTML sinks, exclude only retired source trees through the checked-in CodeQL config, and let GitHub close findings only after the repaired code is scanned.
120. A client-safe contract and an active UI caller do not prove that a backing API exists. Gate each persistent learning call against an owned protected route, make missing or unreadable evidence visibly non-2xx, keep receipts prompt-free and locally ignored, and allow reusable lessons to enter runtime context only after repeated current evidence plus explicit operator approval.
