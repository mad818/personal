...
export function isAutoOpsModeEnabled(
  mode: Settings["officeOperationalMode"],
  settings: Settings,
): boolean {
...
export function getAutoJobsForMode(
  mode: Settings["officeOperationalMode"],
): AutoModeJob[] {
  if (mode === "war" || mode === "nightOps")
    return AUTO_MODE_JOBS.filter((j) => j.mode === mode);
  return [];
}
