...
function useTimelineEvents(): TimelineEvent[] {
  ...
  return useMemo(() => {
    ...
    earthquakes
      .slice(0, 15)
      .forEach((eq: Record<string, unknown>, i: number) => {
        const props = (eq.properties as Record<string, unknown>) ?? {};
        const mag = typeof props.mag === "number" ? props.mag : 0;
        const place =
          typeof props.place === "string" ? props.place : "Unknown location";
        const ts =
          typeof props.time === "number" ? props.time : now - i * 60000;
        if (mag < 2) return;
        events.push({
          id: `eq-${i}`,
          type: "seismic",
    ...
    threatIntel.threatfox
      .slice(0, 10)
      .forEach((item: Record<string, unknown>, i: number) => {
        const name =
          typeof item.ioc === "string"
            ? item.ioc
        ...
        const tsStr =
          typeof item.first_seen === "string" ? item.first_seen : "";
    ...
    hackerNews
      .slice(0, 5)
      .forEach((story: Record<string, unknown>, i: number) => {
        const title =
          typeof story.title === "string" ? story.title : "Tech news";
        events.push({
          id: `hn-${i}`,
          type: "intel",
