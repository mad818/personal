...
function BiasDot({ score }: { score: number }) {
  const color = score > 0.15 ? '#10b981' : score < -0.15 ? '#ef4444' : '#6875a0'
...
function SlidePanel({ topic, articles, savedIds, toggleSaveArticle, onClose }: SlidePanelProps) {
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              ...
              cursor: 'pointer', color: 'var(--text3)', fontSize: '16px',
            ...
            return (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '5px' }}>
                  {a.src && (
                    <span style={{ fontSize: '9px', fontWeight: 700, color: 'var(--text2)' }}>{a.src}</span>
                  )}
                  <BiasDot score={score} />
...
function HeatCell({ topic, articles, maxCount, onClick }: CellProps) {
  ...
  const avgBias = count === 0 ? 0 : articles.reduce((sum, a) =>
    sum + biasScore(a.title + ' ' + (a.desc ?? '')), 0
  ) / count

  const biasColor = avgBias > 0.1 ? '#10b981' : avgBias < -0.1 ? '#ef4444' : '#6875a0'
  const biasLabel = avgBias > 0.1 ? 'Bullish' : avgBias < -0.1 ? 'Bearish' : 'Neutral'
  ...
  const glowOpacity = 0.08 + intensity * 0.30
  ...
  return (
    <button
      ...
      onMouseLeave={(e) => {
        const el = e.currentTarget as HTMLButtonElement
        el.style.transform = ''
        el.style.boxShadow = intensity > 0.4
          ? `0 0 20px color-mix(in srgb, ${topic.hue} ${Math.round(intensity * 25)}%, transparent)`
          : 'none'
      }}
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        <span style={{ fontSize: '12px', fontWeight: 800, color: 'var(--text)', textTransform: 'uppercase', letterSpacing: '.05em' }}>
          {topic.label}
        </span>
      </div>
      ...
      <div style={{ fontSize: '32px', fontWeight: 900, lineHeight: 1, color: count === 0 ? 'var(--text3)' : 'var(--text)' }}>
        {count}
        <span style={{ fontSize: '11px', fontWeight: 500, color: 'var(--text3)', marginLeft: '4px' }}>
      ...
      {count > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: 'auto' }}>
          <span style={{ fontSize: '9px', fontWeight: 700, color: biasColor, display: 'flex', alignItems: 'center', gap: '3px' }}>
            <span style={{ width: '5px', height: '5px', borderRadius: '50%', background: biasColor, display: 'inline-block' }} />
            {biasLabel}
          </span>
          {freshest && (
      ...
      <div style={{
        ...
        <div style={{
          height: '100%', width: `${Math.round(intensity * 100)}%`,
          background: topic.hue, borderRadius: '2px',
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
          <span style={{ fontSize: '11px', color: 'var(--text3)' }}>
            {articles.length} articles across {TOPICS.filter(t => byTopic[t.key].length > 0).length} topics
          </span>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <span style={{ fontSize: '9px', color: 'var(--text3)' }}>Low → High volume</span>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginLeft: 'auto' }}>
            <span style={{ fontSize: '9px', color: '#10b981', fontWeight: 700 }}>● Bullish</span>
            <span style={{ fontSize: '9px', color: '#6875a0', fontWeight: 700 }}>● Neutral</span>
            <span style={{ fontSize: '9px', color: '#ef4444', fontWeight: 700 }}>● Bearish</span>
          </div>
        </div>
      </div>
