...
function syntheticRow(current: number, variance = 12): number[] {
  ...
  return Array.from({ length: 6 }, (_, i) => {
    ...
    return Math.min(
      100,
      Math.max(0, Math.round(base + (current - base) * progress + noise)),
    );
  });
}
...
export default function ThreatHeatmap() {
  ...
  const cyberScore = useMemo(() => {
    ...
    return Math.min(
      100,
      Math.round((high / Math.max(1, threatIntel.threatfox.length)) * 200),
    );
  }, [threatIntel]);
                ...
                return (
                  <motion.div
                    ...
                    style={{
                      ...
                      animation: isHigh
                        ? "nex-pulse 2s ease-in-out infinite"
                        : "none",
                    }}
                  >
                    {cell.value}
