...
export function SystemMonitor({ activeAgent }: SystemMonitorProps) {
  ...
  useEffect(() => {
    const load = () => getMemoryStats().then(st => setMemCount(st.total)).catch(() => {})
    load()
  ...
  const rows: [string, string | number, string][] = [
    ...
    ['🎯 Active',  activeAgent ? AGENTS[activeAgent].name : '—',
  ...
  return (
    <div style={{
      ...
      padding: '8px 8px 8px', borderRadius: '10px',
      background: 'var(--surf3)', border: '1px solid var(--border)',
      width: '100%',
    }}>
      <span style={{ fontSize: '7px', fontWeight: 900, color: 'var(--text3)', letterSpacing: '.16em', marginBottom: '4px', textAlign: 'center', lineHeight: 1, display: 'block' }}>
        SYS
      </span>
      {rows.map(([label, val, color]) => (
        <div key={String(label)} style={{
          ...
          <span style={{ fontSize: '8px', color: 'var(--text3)', lineHeight: 1.2 }}>{label}</span>
          <span style={{ fontSize: '8px', fontWeight: 700, color: color as string, fontFamily: 'monospace', lineHeight: 1.2 }}>
            {String(val)}
          </span>
        </div>
      ))}
