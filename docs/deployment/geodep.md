# GeoDeep AI Scan — Local Service Setup

GeoDeep adds an **AI Scan** layer to the Nexus Prime OPS map. It runs a lightweight
object-detection pass over a public satellite tile and plots detected features as
purple markers on the Leaflet map.
...
1. You toggle **AI Scan** on the OPS map.
2. Nexus calls `GET /api/geo-scan`.
3. The API route proxies to `http://localhost:5050/scan`.
4. The Python service fetches one satellite tile (OSM public tiles — free),
   runs a pixel-contrast edge scan (no GPU, no ML model download), and returns
   detected feature centroids as `{ lat, lng, label, confidence }`.
...
Then switch to **AI Scan** in the OPS tab and click **↻ Run AI scan**.
...
- Results are cached in-memory for 5 minutes. Click **↻ Run AI scan** to force a refresh.
