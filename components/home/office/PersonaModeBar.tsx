...
const PERSONA_OPTS: { mode: PersonaMode; label: string; tip: string }[] = [
  {
    mode: "formal",
    label: "F",
    tip: "Formal — structured, cited, institutional",
  },
  {
    mode: "direct",
    label: "D",
  ...
  },
  {
    mode: "deep",
    label: "∞",
    tip: "Deep — exhaustive, multi-angle, full reasoning",
  },
];
...
export function PersonaModeBar() {
  ...
  return (
    ...
    >
      <span
        style={{ color: "var(--text2)", fontSize: "10px", marginRight: "2px" }}
      >
        MODE
      </span>
      {PERSONA_OPTS.map(({ mode, label, tip }) => (
        <button
          ...
          style={{
            ...
            background:
              activePersona === mode ? "var(--accent)" : "transparent",
