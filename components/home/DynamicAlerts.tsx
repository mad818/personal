...
import { UI_RULES, resolveTemplate, buildSnapshot } from "@/lib/uiRules";
...
function FloatCard({ rule, snapshot, onDismiss }: {
  ...
  if (!card) return null;
  const title = resolveTemplate(card.title, snapshot);
  const body  = resolveTemplate(card.body,  snapshot);
...
function NavBadge({ rule, snapshot }: { rule: UIRule; snapshot: ReturnType<typeof buildSnapshot> }) {
  ...
  const label = resolveTemplate(rule.badge.label, snapshot);
...
export function DynamicAlerts() {
  ...
  const activeRules = activeUIRuleIds
    .map((id) => UI_RULES.find((r) => r.id === id))
    .filter((r): r is UIRule => !!r);

  const floatCards = activeRules.filter((r) => r.action === "float-card");
  const navBadges  = activeRules.filter((r) => r.action === "nav-badge");
  ...
  return (
    <>
        ...
        >
          {floatCards.map((rule) => (
            <FloatCard
              ...
              rule={rule}
              snapshot={snapshot}
              onDismiss={() => dismissUIRule(rule.id)}
            />
          ))}
        </div>
      )}
      ...
      {navBadges.map((rule) => (
        <NavBadge key={rule.id} rule={rule} snapshot={snapshot} />
      ))}
    </>
  );
}
