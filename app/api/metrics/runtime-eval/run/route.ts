...
function runCommand(cmd: string, args: string[], timeoutMs: number): Promise<{ ok: boolean; output: string }> {
...
export async function POST(req: Request) {
  ...
  const baseCooldownMin = Math.max(1, Math.min(24 * 60, Number(process.env.NEXUS_RUNTIME_EVAL_COOLDOWN_MIN ?? '30')))
  const prev = readRunnerState()
  const priorFailureStreak = Number(prev.failureStreak ?? 0)
  const effectiveCooldownMin = Math.min(24 * 60, baseCooldownMin * Math.max(1, 2 ** priorFailureStreak))
  const now = Date.now()
  const lastRunMs = prev.lastRunAt ? new Date(prev.lastRunAt).getTime() : 0
  const elapsedMin = lastRunMs > 0 ? (now - lastRunMs) / 60000 : Number.POSITIVE_INFINITY
  const eligible = elapsedMin >= effectiveCooldownMin

  if (!eligible && !force) {
    const nextEligibleAt = new Date(lastRunMs + effectiveCooldownMin * 60000).toISOString()
  ...
  const r = await runCommand('npm', ['run', 'eval:agent-runtime:record'], 300_000)
  const nextFailureStreak = r.ok ? 0 : Math.min(priorFailureStreak + 1, 8)
  const nextEffectiveCooldownMin = Math.min(24 * 60, baseCooldownMin * Math.max(1, 2 ** nextFailureStreak))
  const summary = r.ok ? 'Recorded runtime eval successfully' : `Runtime eval failed: ${r.output.slice(0, 120)}`
  const nextEligibleAt = new Date(Date.now() + nextEffectiveCooldownMin * 60000).toISOString()
  writeRunnerState({
