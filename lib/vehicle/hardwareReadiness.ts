...
export const VEHICLE_CONNECTOR_TRANSPORT_LABELS: Record<
  VehicleConnectorTransport,
  string
> = {
...
export const VEHICLE_RENDER_BRIEF_TARGET_LABELS: Record<
  VehicleRenderBriefTarget,
  string
> = {
...
export const VEHICLE_BAUD_RATE_OPTIONS = [
  57_600, 115_200, 230_400, 460_800, 921_600,
] as const;
  ...
  [
    {
      ...
      detail:
        "No first-day integration milestone is worth spinning props indoors or during setup.",
    },
    {
      ...
      detail:
        "Validate the autopilot on its native ground station before introducing the bridge observer.",
    },
    {
      id: "port-and-baud",
      category: "bridge",
      ...
      detail:
        "Arrival-day friction usually starts with the wrong cable, COM port, or baud assumption.",
    },
    {
      ...
      detail:
        "Nexus should ingest telemetry only; it should not be able to arm, mode-switch, or steer the aircraft.",
    },
    {
      ...
      detail:
        "Use the first minute to compare the bridge view against the ground station and catch mapping drift.",
    },
    {
      id: "compliance-review",
      category: "compliance",
      label:
        "Run FAA / Remote ID / local-airspace review for the real mission area",
      detail:
        "Once the airframe exists, compliance should move from theory to the actual city, airport context, and operation type.",
    },
    {
      ...
      detail:
        "The arrival-day session should leave a concrete JSON bundle before any later tuning muddies the baseline.",
    },
    {
      ...
      detail:
        "Treat the first hardware day like a reusable runbook, not a one-off memory.",
    },
  ];
  ...
  [
    {
      ...
      trigger:
        "Ground station sees the aircraft but Vehicle Lab stays on simulation.",
      steps: [
    ...
    },
    {
      ...
      trigger:
        "Frames arrive once, then freshness ages out and Vehicle Lab drops back to simulation.",
      steps: [
    ...
    },
    {
      ...
      trigger:
        "Pitch/roll/yaw, heading, or home data disagrees with the ground station.",
      steps: [
...
function normalizeVehicleRadarSummary(
  ...
  const modeLabel = trim(
    typeof value.modeLabel === "string" ? value.modeLabel : "",
    60,
  );
  const summary = trim(
  ...
  const fusionNote = trim(
    typeof value.fusionNote === "string" ? value.fusionNote : "",
    200,
  );
  ...
  return {
    ...
    fusionNote:
      fusionNote || "Sensor-fusion note still pending operator review.",
    artifactLabels,
  };
}

export function normalizeVehicleConnectorProfile(
  value: unknown,
  ...
  const baudRate =
    ...
    VEHICLE_BAUD_RATE_OPTIONS.includes(
      input.baudRate as (typeof VEHICLE_BAUD_RATE_OPTIONS)[number],
    )
      ? input.baudRate
...
export function areVehicleChecklistStatesEqual(
  ...
  return itemIds.every(
    (itemId) => Boolean(left[itemId]) === Boolean(right[itemId]),
  );
}
...
function buildChecklistProgress(
  ...
  return {
    ...
    remainingCount,
    nextIncompleteLabel:
      items.find((item) => !checklistState[item.id])?.label ?? null,
  };
}
  ...
  const gates: VehicleBenchBridgeGate[] = [
    {
      ...
      detail:
        "Frame power, battery restraint, and first-day props-off posture are confirmed before any bridge work.",
      complete:
        bench("props-removed") &&
        bench("battery-restraint") &&
        firstDay("props-off-arrival"),
      proof:
        "Props removed + battery restrained + first hardware day stays props-off.",
    },
    {
      id: "native-gcs-first",
      label: "Ground station proves heartbeat first",
      detail:
        "Mission Planner or QGroundControl sees the aircraft before Nexus is allowed to observe it.",
    ...
    },
    {
      ...
      detail:
        "IMU axes, compass, RC channels, and mode switches match the ground station.",
      complete:
        ...
        bench("mode-switches"),
      proof:
        "FC orientation, compass, RC mapping, and mode transitions checked.",
    },
    {
      id: "gps-home-failsafes",
      label: "GPS, home, and failsafes are reviewed",
      detail:
        "Home point, GPS quality, radio failsafe, and battery failsafe are understood before field work.",
      complete:
        bench("gps-lock") &&
        bench("home-point") &&
      ...
      proof:
        "3D GPS, believable home, radio failsafe, and battery threshold reviewed.",
    },
    {
      id: "passive-bridge-only",
      label: "Bridge cannot become flight authority",
      detail:
        "The saved connector profile and first-day checklist both keep Nexus in observer mode.",
      complete:
        readOnlyBridge &&
    ...
    },
    {
      ...
      detail:
        "The first telemetry session is exported and filed before later tuning changes the baseline.",
      complete:
        firstDay("export-first-session") && firstDay("file-vault-summary"),
...
export function buildVehicleBridgeStubCommand(
...
export function buildVehicleConnectorProfileJson(
  profile: VehicleConnectorProfile,
) {
...
export function buildVehicleFlightSessionBundle({
  ...
  return {
    ...
    firstHardwareDayChecklist: getVehicleFirstHardwareDayProgress(
      firstHardwareDayChecklistState,
    ),
  };
}

export function parseVehicleFlightSessionBundle(
  raw: unknown,
):
  ...
  if (
    ...
    typeof parsed.manifest.sessionLabel !== "string"
  ) {
    return { ok: false, message: "Bundle is missing a valid manifest." };
  }
  if (
    !isRecord(parsed.activeFrame) ||
    typeof parsed.activeFrame.timestamp !== "number"
  ) {
  ...
  return {
    ...
    bundle: {
      ...bundle,
      connectorProfile: normalizeVehicleConnectorProfile(
        bundle.connectorProfile,
      ),
...
export function buildVehicleSessionVaultDraft(
  ...
  const transportLabel =
    VEHICLE_CONNECTOR_TRANSPORT_LABELS[connector.transport];
  ...
  return {
    ...
    topic: bundle.radar
      ? "Vehicle session summary · radar readiness"
      : "Vehicle session summary",
  };
}
...
export function buildVehicleRenderBrief({
  ...
  return [
    ...
    operatorGoalLine
      ? `- Operator goal: ${operatorGoalLine}`
      : "- Operator goal: not specified yet; keep the first concept conservative and easy to revise.",
...
export function buildVehicleRenderBriefVaultDraft({
  ...
  return {
    ...
    topic: bundle.radar
      ? "Vehicle render brief · radar-ready"
      : "Vehicle render brief",
  };
}
