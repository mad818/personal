...
export default function HealthMonitor(): React.ReactElement {
  ...
  const updateSystem = useCallback(
    (id: string, patch: Partial<SubsystemInfo>) => {
      setSystems((prev) =>
        prev.map((s) =>
          s.id === id ? { ...s, ...patch, lastCheck: Date.now() } : s,
        ),
      );
    },
    [],
  ...
  const checkStorage = useCallback(() => {
    ...
    const status: SubsystemStatus =
      pct > 90 ? "down" : pct > 70 ? "degraded" : "healthy";
  ...
  useEventBus("system:health", (data) => {
    ...
    const id = data.component.startsWith("ws:")
      ? "websocket"
      : data.component === "ollama"
        ? "ollama"
        : data.component.startsWith("plugin:")
          ? null
          : null;
    if (id) {
      updateSystem(id, {
              ...
              >
                ...
                <span
                  style={{
                    ...
                    boxShadow:
                      sys.status === "healthy"
                        ? `0 0 6px ${STATUS_COLOR[sys.status]}88`
                        : "none",
                    animation:
                      sys.status === "checking"
