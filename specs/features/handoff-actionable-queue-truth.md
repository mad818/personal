...
Keep the canonical Codex handoff from presenting blocked, manual, remote-only, or excluded RPG tasks as immediate work when the repository already has a deterministic actionable-queue classifier.
...
- When none exist, the handoff must say that no locally actionable non-RPG task is currently proven.
- In the generated `What’s next` section, blocked/manual and RPG-excluded work may appear only as bounded counts and review guidance, not as promoted task titles; existing non-queue continuation notes are outside this selector.
...
- Runtime fixtures cover mixed actionable/blocked/RPG queues, zero-actionable queues, output ordering, the three-item cap, and suppression of blocked/excluded task titles.
