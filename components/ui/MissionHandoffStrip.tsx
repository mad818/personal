...
function getMissionHandoffContent(
  ...
  if (surface === "command" && handoff.mission === "observe") {
    return {
      summary:
        "Observe continues here. COMMAND is the first scan for runtime posture, live pressure, and briefings before you branch wider.",
      detail:
    ...
    return {
      summary:
        handoff.source === "intel"
          ? "Investigation continues here. INTEL carries the active signal front from HQ into narrative, world, market, and sweep context."
          : "Investigation continues here. INTEL is the wider evidence lane when HQ needs more signal before action.",
      detail:
      ...
      contextHint:
        handoff.source === "intel"
          ? "Best next: follow the active signal front"
  ...
  if (surface === "cyber" && handoff.mission === "investigate") {
    return {
      summary:
        "Investigation continues here. CYBER is the containment lane for triage, correlated threat signals, and exposed posture.",
      detail:
  ...
  if (surface === "security" && handoff.mission === "investigate") {
    return {
      summary:
        "Investigation continues here. SECURITY is the control lane for route, auth, tool, AI-surface, and physical-ops review before or after implementation work.",
      detail:
        "Start in controls or AI surface review to frame the risk properly, then widen into physical monitoring only if the session really needs the operational side.",
      tone: "caution",
      contextHint: "Best next: controls + AI surface",
    };
  }
